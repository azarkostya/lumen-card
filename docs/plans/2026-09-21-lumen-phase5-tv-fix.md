...
- [ ] Открытый вопрос координатору: обёртка встаёт при активации плагина, а первый экран главной Lampa собирает по `setTimeout(last, 500)` из `Activity.init` (`app.min.js:45641`) — если плагин догрузился позже, первая главная строится без дедупликации (и без наших рядов вовсе, это не ново). Разбор вариантов — в отчёте по фикс-раунду; принудительной пересборки пока НЕ делали.
