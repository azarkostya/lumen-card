...
## 5. Фазы 2 и 3 — принятые решения (детализация в задачи после ресёрча `API_NOTES_4.md`)

Фаза 1 — карточка (Task 3–13 выше). Дальше две фазы, задачи будут дописаны в этот файл тем же форматом, когда придёт ресёрч главной и подборок. Что уже решено:
