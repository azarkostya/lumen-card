...
> - (4) и (5) — **открыты**, перенесены в раздел D финального плана (`docs/plans/2026-09-22-lumen-final.md`). Мест с (5) по-прежнему 4: `src/55_trailer.js:119`, `src/81_prefs.js:655`, `src/90_runtime.js:1105` и `:1298`.
> - (6) — **открыт**: `find()`/`children()` фейка по-прежнему отдают первый элемент (`test/_fakedom.mjs`). Родственная дыра того же фейка — он не разбирает комбинаторы селектора, из-за чего ширина полосы «Продолжить» не проверяется ни одним тестом (раздел D финального плана).
