...
- **Кнопки и хэш приоритета**: в `full.js` порядок синхронный — `Listener.send('full', {type:'complite'})` → `items[0].emit('groupButtons')` → `activity.toggle()`. `groupButtons` сравнивает `Lampa.Utils.hash($(btn).clone().removeClass('focus').prop('outerHTML'))` с сохранённым `Storage 'full_btn_priority'`. Online Mod (`nb557.github.io/plugins/online_mod.js`) на том же `complite` делает `e.object.activity.render().find('.view--torrent').after(btn)` с кнопкой `.full-start__button.selector.view--online_mod` и своим svg. Поэтому **outerHTML кнопок менять нельзя никогда** (ни в шаблоне, ни после вставки): любое изменение в зависимости от порядка обработчиков сбивает приоритетную кнопку пользователя. Иконки, цвета и подписи кнопок — только через CSS.
  ...
  **События активностей Lampa 3.3.4 (найдено в Task 6, исходник + живой лог):** `Lampa.Activity.push` НЕ шлёт события для покидаемой активности; `backward()` шлёт `start`, затем `archive` для активности, к которой ВЕРНУЛИСЬ (`e.object` — её объект), и `destroy` — для удалённой. Поэтому «пауза при уходе вглубь» делается проверкой `closest('.activity').hasClass('activity--active')` в тике таймера, а не подпиской; при `start` полной карточки восстанавливать `LC.active` на вернувшийся объект.
  **Булевы настройки:** `Lampa.Storage.set(name, false)` с JS-значением `false` не сохраняется (`Storage.get`: `value || empty`); переключатели настроек Lampa пишут строки `'true'` / `'false'` — читать через `Lampa.Storage.field` и сравнивать и со строкой, и с булевым; в живых проверках ставить строку.
  **Слой фона вне карточки:** `.lumen-backdrop` — сосед `.lumen-card` в `e.body`, не потомок: его CSS ограничивать `.lumen-backdrop`, класс режима анимаций зеркалить на слой (правило `.lumen-card … .lumen-bg__img` никогда не совпадёт — найдено в Task 6).
  **Скрытая панель браузера (найдено в Task 5a):** когда браузерная панель не видна пользователю, у вкладки `document.visibilityState === "hidden"` и CSS-transition/animation не проигрываются — `getComputedStyle` отдаёт исходное значение (например, открытый `Lampa.Select` «стоит» за краем экрана, x = 1920). Это не дефект: для проверок положения и трансформаций читать целевое значение, временно выставив элементу `style.transition = "none"`, а после чтения **восстановить атрибут точно**: если у элемента не было атрибута `style` — `el.removeAttribute("style")` (присваивание `""` оставляет пустой `style=""`, это меняет outerHTML и сбивает хэш кнопок — найдено в Task 5a). На кнопках карточки этот приём применять только после снятия хэшей или на `cloneNode(true)`, вставленном рядом. Проверено: `.selectbox__content` при `body.selectbox--open` — `translate3d(-100%,0,0)`, x = 1248, ширина 672 при 1920 px.
...
> **ВЫПОЛНЕНО** (6ca044c, 5c99e1a; сверка PASS, ревью качества Approved with minor): `LC.cardinfo.nextEpisode/network/shortDate` (словарь строк параметром, ru/en/uk), `LC.progress.episodeState` (watched/watching/aired/soon, percent, leftMin), мета `networks[0].name`, подзаголовок `original_name · создатель`, чип следующей серии, `.lumen-card--serial` (статус переносится в ленту рейтингов, каст скрыт), ряд серий последнего сезона `.lumen-episode` с кадрами w300 окном ±6, прокрутка к фокусной карточке, `.lumen-compact` по фокусу, OK на серии → `.button--play`, подписка `LC.followTimeline` перерисовывает состояния; `test/header.test.mjs` на связующий код. 364 теста, стенд 31/31, хэши 7/7. Долг ревью (кадры за окном не выгружаются; сверка списка серий только по ссылке на массив; мелочи фейкового DOM) — в Task 5d. Не сделано: подписи «E3 · СМОТРИТЕ» / время / «Выходит» в сжатой шапке (экран 06) — перенесено в Task 8.
...
> **ВЫПОЛНЕНО** (a76fa5d долг 5c, a72cc61, 6946649, 1d36042; сверка PASS — 0 расхождений по §10, ревью качества Approved with minor ×2): `LC.cardinfo.facts(movie, persons, words)` (чистая, подписи параметром), рендер `renderDescrRow` вставляет `.lumen-facts` в `.full-descr` (идемпотентно, без `.selector`, пропуск повторного рендера по подписи), подписки `build`+`description` и страховочная `complite`, класс-корень `.lumen-descr-row` на одном `.items-line`; штатный `.full-descr__details` и заголовок ряда `> .items-line__head` скрыты CSS; у описания снята маска Lampa, предел высоты 70vh (с `none` фокусируемые `.full-descr__tags` уходят за экран — Lampa прокручивает только при смене ряда, замер: теги top 3342 при вьюпорте 1080); зазоры сетки в старом и новом синтаксисе, `-ms-grid` снят, flex-фолбэк под тестом. Долг Task 5c закрыт (`dropStills` — выгрузка кадров за окном, `episodesSign` — сверка списка серий). 416 тестов, стенд 31/31, хэши 7/7. Формат «Время» `2:46` зафиксирован в `design-spec-card.md` §10.
...
> **ВЫПОЛНЕНО** (027f37d, c08c06f, 901896f, 3a94be3; сверка PASS по экранам 09/14, ревью качества ×2 → Approved with minor): 13 настроек и 4 заголовка групп в `LC.prefs.LIST` (`src/81_prefs.js`), у каждой своё применение на лету и ветка в `followStorage` (возврат из настроек Lampa не перерисовывает карточку); `lumen_enabled` — `activate()`/`deactivate()`/`stripAllCards()`: возврат оригинального шаблона, снятие обоих `<style>`, `<link>` шрифтов, классов `body` и карточек, узлов, CSS-переменной подписи, гашение слоёв фона и запросов отзывов ПО ВСЕМУ документу (не только у активной карточки), гейт `activated` в `LC.onActivityEvent`/`followToggle`/`followTimeline`; `LC.applyKpRate(rate, row)` привязан к запросившей карточке; одна пересборка CSS на изменение (`pref_handled`); кольца акцентов ice/wine/mint выправлены по экспорту. Закрыты долги: `onChange` как страховка при падении чужого подписчика Storage, полнота тестов применения, регресс-тест фолбэков масок, `NO_MASK` выше `css()`, мок Storage в стенде пишет в `localStorage` (+ переключатель плагина и чистка `lumen_rv_*`). 626 тестов, стенд FAIL 0, хэши 7/7.
>
> **Правка пользователя 2026-09-16 (читаемость):** таблица «ПОДРОБНО» не читалась поверх светлых кадров. Добавлены подложка `rgba(11,9,8,.85)` с рамкой и растушёванная вуаль ряда, подписи `smoke` → `muted`, кегль `.79em` → `.877em`, ритм сжат; тем же приёмом подняты `.lumen-reviews__total`/`__ico` и мета отзывов. Контраст подписи к подложке 7.1–7.2:1 (было 1.03:1 на «дюнном» кадре), блок занимает ~24.8 % высоты на 720p/1080p/4K. Отступление от §10 зафиксировано в `design-spec-card.md`.
...
> **ВЫПОЛНЕНО** (2287195, b9684f7; сверка PASS, ревью качества Approved with minor): `LC.destroyActive()` сводит освобождение ресурсов карточки в одну точку поверх `LC.onActivityEvent` (обнуление `LC.active` первым, каждое освобождение в своём try/catch): предзагрузка кадра, ротация слайдшоу, таймер остывания, трейлер (старт 3 с, таймаут 6 с, сторож 1 с, iframe), `lumenReviveCleanup`, запрос отзывов, кадры серий; прямые `destroy` слайдшоу и трейлера как страховка, если слоя в теле нет. Колбэки после уничтожения обезврежены, включая колбэки YouTube (гейт `dead`). Ошибка подмены шаблона на `init` откатывает активацию целиком (тест ловит отсутствие `return`). На `backward` ресурсы покидаемой карточки снимаются на `start` возвращаемой, а не по тику ротации (было до 8–20 с лишних CPU и сети). Живые замеры: 5 кругов «Дюна», 5 кругов «Фоллаут», push×2+backward×2, уход вглубь с трейлером — интервалы и таймауты возвращаются к нулю, XHR 0, узлы 0. 637 тестов, стенд 48 проверок FAIL 0, хэши 7/7. Отдельный интервал-страж НЕ добавлялся — его роль выполняют `isMounted`/`isLayerMounted` в тиках.
...
> **ВЫПОЛНЕНО** (5f88dfc, c04ba61, 48db086, 5828356; ревью кода фазы по снимку b9684f7 → «Approved with minor», подтверждающее ревью 48db086 → «отправлять можно, фазу закрывать можно», живая приёмка в vendor Lampa 12/12 PASS): закрыты все находки ревью фазы. I1 — `backdrop-filter` гасится в `lumen-motion-lite/-off` у кнопок, «Стоп» и метки трейлера, а полупрозрачные заливки «Стоп» (.5) и метки (.62) при этом уплотняются до `rgba(BG_RGB,.9)`: поверх светлого кадра ролика контраст `C.text` рос бы 3.21:1 (ниже порога), стало 13.69:1; фокус держится специфичностью (0,4,0) против (0,3,0), это проверяется тестом веса селекторов, а не подстрокой. I2 — осиротевшая карточка (вытеснение по `maxsave`) освобождается так же, как своя: `backdrops.cancel` + `reviews.cancel`, каждое в своём try/catch. I3 — `LC.init` идемпотентен: флаг `inited` со сбросом в `catch` (неудачный запуск больше не необратим) плюс собственные флаги у подписки `full` (`followFull`), `LC.addSettings` (`settingsAdded`) и `LC.followStorage` (`storageFollowed`). M1 — кадры не поднимаются под играющим роликом на всех трёх входах (`applySlideshowPref`, ветка `archive|start`, восстановление карточки из истории) через единую точку `LC.trailer.isLive(layer)` (класс `lumen-trailer-live`, ставит `onStart`, снимает `cleanup()`); промежуточный гейт по живости контроллера давал регресс «слайдшоу замерло до переоткрытия» и заменён. M2 — сообщение о неподдерживаемой сборке в `LC.STRINGS`. M5 — комментарий `stripAllCards` приведён к фактам. Мелкое: `typeof body.children === 'function'` в обоих местах, единый источник корней в `test/css.test.mjs` (`ALLOWED_ROOTS`), стенд нормализует `lumen_enabled` через `prefs.boolOf`. 663 теста, `dist is up to date`, ES5 ok, хэши 7/7.
>
> Живая приёмка (только vendor Lampa, стенд не использовался): ряд серий «Фоллаут» 106379 — 8 карточек, окно кадров `STILL_WINDOW=6`, состояния на настоящем `Lampa.Timeline`, навигация ↓/←/→/↑ и компактная шапка; путь до плеера на 20 НАСТОЯЩИХ раздачах (jacred.xyz): Select «Источник» с `data-lumen-kind`, экран `.lumen-torrents`, меню раздачи, окно подключения и чек-лист ошибки, список файлов, `media-loading` с пилюлей статуса; трейлер — iframe на 3.5–4 с, откат по таймауту 6 с, снятие по OK, `off` не грузит ни iframe, ни API; отзывы — фокус → модал → возврат на тот же узел; шапка «Дюны» с режиссёром на живых данных, смена кадров по интервалу, Ken Burns; таблица «Подробно» 6 строк читается поверх светлого закатного кадра; `lumen_enabled='false'` — 0 наших узлов, оба `<style>` и классы сняты; хэши 7/7 против эталона, снятого в той же Lampa БЕЗ плагина; раскладки 1280×720 и 3840×2160 (карточка серии ровно 14.9em в обеих); консоль без ошибок плагина.
>
> Ложные тревоги первой приёмки (прогон был сделан на стенде вместо живой Lampa): «`.lumen-reviews` остаётся после выключения» — стенд звал `reviews.render()` напрямую, в обход гейта `activated`, и возвращал узел в уже раздетую карточку (стенд исправлен); «подписи 14px вместо 20» — у стенда базовый кегль 16px против 22.811 в полноэкранной Lampa, правило `.88em` верное.
>
> **НЕ проверялось:** реальные Tizen / webOS / Android TV (только браузерная Lampa); настоящее воспроизведение ролика (в скрытой панели браузера autoplay заблокирован — проверены создание iframe, загрузка API, откат по таймауту); успешное подключение к TorrServer и реальная предзагрузка (рабочего сервера в сети нет); отзывы на настоящем ключе Кинопоиска (по решению пользователя ключ не получаем); состояние серии «скоро» (у «Фоллаута» сезон вышел целиком).
>
> **Долги, вынесенные в фазу 2:** (1) `activate()` поднимает `activated` первой строкой и откатывает только при неудачной подмене шаблона — падение `injectFonts`/`injectCss` оставит плагин «активным» без CSS, а повторный `activate()` выйдет первой же строкой; (2) ↑ с дальней правой серии уходит в верхнее меню Lampa мимо кнопок карточки (геометрия штатного `Navigator`, наш код только двигает дорожку); (3) `COUNTRY_RU` (`src/35_cardinfo.js`) — русские названия стран при любом языке интерфейса, мимо `LC.STRINGS`; (4) пять способов ответить на вопрос «наша карточка на экране» (`applyReviewsPref` остался на глобальном `.activity--active`); (5) `LC.pref` против `Lampa.Storage.field` в 4 местах; (6) `test/_fakedom.mjs` — `find()`/`children()` возвращают ПЕРВЫЙ элемент, а не набор, поэтому `stripAllCards`/`refreshProgress`/`refreshEpisode` тестируются на одном элементе; (7) модалка «Файлы» не получает класс `lumen-modal` (`isPathModal` метит только `.modal-loading`/`.torrent-install`) — визуально ни на что не влияет.
>
> **Статус долгов на 2026-09-23** (каждый перепроверен по коду):
> - (1) **не долг в этой форме.** `LC.injectFonts` и `LC.injectCss` свои исключения глотают сами (`src/30_css.js:4726-4756` и `4773-4798`, `try/catch` вокруг всего тела), поэтому `activate()` (`src/90_runtime.js:1272`) на них не обрывается и проходит до конца; откат на неудачной подмене шаблона на месте (`:1280-1287`). Остаётся один сценарий — `LC.buildCss` бросает, и плагин активен без своей таблицы стилей; повторный `activate()` его бы не исправил (та же ошибка), а `LC.injectCss` и так зовётся заново при каждой смене настроек.
> - (2) **закрыт `8ece232`** (2026-09-23): Navigator Lampa берёт цель «вверх» только из прямой полосы над плиткой, а ряд серий шире ряда кнопок; обёртка `up` контроллера `full_start` ставит фокус на ближайшую по горизонтали кнопку. Кнопки не переставлялись.
> - (3) **закрыт `7c7c822`** (страна в мета-строке — на языке интерфейса).
> - (4) **закрыт `78d0bdb` + `34c8d94`** (2026-09-23): пересчёт нашёл не пять, а шесть записей правила «ближайшая .activity помечена activity--active» (глобальный селектор в `src/90_runtime.js` и `src/69_hud.js`, `LC.slideshow.isLayerForeground`, две копии `isForeground` в `src/60_reviews.js` и `src/66_franchise.js`, `archived()` в `src/52_fx.js`, `closest('.activity--active')` в `src/49_moods.js`); все сведены к `LC.util.onScreen` (`src/10_util.js`). Поведение не изменилось — тесты `78d0bdb` зелёные и на прежнем `src/`, и после. `LC.active` и «узел ещё в документе» — отдельные вопросы, разбор у функции.
> - (5) **закрыт `44086b1`** (2026-09-23): мест было пять — к четырём названным добавился `src/68_perf.js` (`motionRaw`, `st.field('lumen_motion')`); все читают `LC.pref` с дефолтом пункта. Оставшиеся четыре `Storage.field` читают настройки самой Lampa, у каждого комментарий почему; сторож — `test/prefs.test.mjs`.
> - (6) **закрыт `d0738c0`** (2026-09-23): `find()`/`children()` фейка отдают все совпадения, как jQuery (набор `FakeSet`). Родственная дыра — комбинатор «>» и голый тег — закрыта раньше, `7a0e93e` (ширина полосы «Продолжить» под тестом).
> - (7) **не долг.** Список файлов TorrServer Lampa показывает в той же модалке, что и загрузку: `loading()` и сразу `files()` (`vendor/lampa/app.min.js:40386-40388`), а `files()` меняет содержимое через `Modal.update` (`:40894-40896`), который пересобирает только прокрутку и оставляет корень `.modal` прежним (`:32534-32549`). Маркер `lumen-modal`, поставленный на `fullshow` загрузки (`src/64_menus.js:169-178`, `isPathModal` — `:80-83`), на корне остаётся и для «Файлов».
...
**Вспомогательная задача (вне нумерации), выполнена 2026-09-16 (8107f6c, 2158196):** `dist/lumen_card.js` собирается со снятием комментариев и отступов — `scripts/lib/strip.mjs` (acorn, `ecmaVersion: 5`, замена комментария на столько же переводов строки, поэтому нумерация строк сохраняется), 289 → 139 КБ. Сохраняются шапка `/*!` и маркеры `/* ---- NN_имя.js ---- */` (по ним `build` и `es5check` показывают координаты в `src/`). Проверено: AST и поток токенов до/после идентичны. В `src/*.js` нельзя вставлять U+2028/U+2029 (рвут regexp-литералы; при их наличии сжатие отключается). Тесты — `test/strip.test.mjs`, `test/build.test.mjs`.
