...
> **ВЫПОЛНЕНО** (`99eb024`, `076be56`, `48579ff`, каталог расширен в `a89cae3`): `src/42_manifest.js` — встроенный каталог (147 подборок, 10 групп, 4 профиля настроения), `validate` / `orderForMonth` / `isFresh` / `load` с кэшем 12 ч в `Lampa.Storage` и фолбэком на встроенный; `src/43_sources.js` — `buildRequest` / `normalize` / `discoverUrl` / `mergeMedia` / `fetch`, ветка Кинопоиска (`films/collections` → `find/{imdbId}`, кэш 30 дней, без ключа — `err({nokey:true})` без единого запроса), негативный кэш и подписка на запрос в полёте. `manifest.json` в корне генерируется из модуля `scripts/manifest.mjs` при каждой сборке — файл и код не расходятся. Проверено тестами `manifest`/`sources`/`sources_network` и живой загрузкой подборки на локальной Lampa 3.3.4 (`star-wars` — 11 карточек, первая «Эпизод 4»). Долг: адрес хостинга (`LC.MANIFEST_URL`) заполнен в Task 20.
...
> **ВЫПОЛНЕНО** (`3241064`, `c60d792`): `src/44_rows.js` — `homeRows` (состав из настройки или `manifest.home`, сезонные наверх, лимит), `filterWatched` + `viewedIds` (`Favorite` 'viewed' и `Timeline` ≥ 95 %), регистрация через `Lampa.ContentRows.add` с `index` от 4 (0–3 отданы персональным рядам), снятие через `ContentRows.remove` при повторной регистрации и деактивации, отмена запросов при уходе с главной по поколению. Настройки `lumen_home_rows` / `lumen_hide_watched` / `lumen_rows_limit`. Проверено тестами `rows` и живьём: ряды выбранных подборок появляются на главной («Звёздные войны», «Аниме»), карточки открываются. Долг: ряды подгружаются лениво — Lampa строит их по мере прокрутки, поэтому на первом экране их не видно (поведение самой Lampa, не плагина).
...
> **ВЫПОЛНЕНО** (`2bbb81b`, `fbcb383`): `src/45_personal.js` — ряды «Досмотреть» (`Favorite.continues`), «Потому что вы смотрели «X»» (рекомендации по двум последним из истории), «Новые серии ваших сериалов» (детали сериалов из закладок и истории, окно −14/+7 дней, бейдж с датой), «Скоро на экранах» (`discover` на 30 дней вперёд, фильмы и сериалы вперемежку); индексы 0–3, пустой ряд не регистрируется, выключатель `lumen_personal_rows`. Проверено тестами `personal` и живьём: ряд «Скоро на экранах» на главной локальной Lampa. Данные берутся только из локального `Lampa.Favorite`/`Timeline` и никуда не отправляются.
...
> **ВЫПОЛНЕНО** (`274dca0`, `3aa8799`): `src/46_hub.js` — пункт «Подборки» в левом меню, компонент `lumen_hub` (чипы групп из `hubGroups`, плитки с коллажом из трёх постеров, ленивая подгрузка коллажей), компонент `lumen_grid` (сортировка «По популярности / По рейтингу / Новые», локальная сортировка для коллекций и списков, догрузка страниц, пустой экран с кнопкой «Назад»), `openTarget` (чисто-`discover` подборки уходят в штатный `category_full`, остальные — в свою сетку), кнопка «Франшиза» в карточке по `belongs_to_collection`. Проверено тестами `hub` и живьём: хаб открывается (34 плитки в чипе «Франшизы»), сетка подборки Кинопоиска и кнопка «Франшиза» в карточке «Дюна: Часть вторая». Долг фазы: в сетке подборки содержимое не прокручивается вместе с шапкой, поэтому мини-герой (Step 4 Task 18) в ней не включён.
...
> **ВЫПОЛНЕНО** (`cfe0047`, `1b57b88`; плюс `05445e0` — просмотр трейлера со звуком штатной кнопкой Lampa): `src/48_hero.js` — слой `.lumen-hero` над рядами главной, карточка под фокусом ловится `MutationObserver` по `.card.focus` (`el.card_data`), дебаунс 350 мс, кроссфейд кадра после предзагрузки, логотип и детали через `tmdb.get(media/id, …, {life:1440})`, компактный режим `.lumen-hero--compact` при уходе фокуса ниже первого ряда, монтирование/снятие по событиям активности и `mountCurrent()` для уже открытой главной. Проверено тестами `hero` и живьём: герой на главной локальной Lampa, кадр и текст меняются по фокусу, наблюдатель снимается при уходе. Долг фазы: герой занимает ~36 % высоты экрана вместо 58 % по дизайну — высоту задаёт отступ первого ряда, а карточки рядов Lampa крупнее дизайнских.
...
> **ВЫПОЛНЕНО** (`f134bad`, `c5290b2`; локализация чипов починена в Task 20): `src/49_moods.js` — блок `.lumen-moods` внутри текста героя, четыре чипа из `manifest.moods`, OK открывает штатную сетку `category_full` с параметрами профиля. Навигация вышла проще плановой: чипы — обычные `.selector`, и пространственный `Navigator` Lampa находит их геометрически, отдельный контроллер не понадобился; после монтирования делается `recollect`, чтобы новые узлы сразу попали в коллекцию. Проверено тестами `moods` и живьём (четыре чипа под текстом героя, названия по-русски). Долги фазы: переход «вверх из первого ряда на чипы» физическим пультом не проверен — только синтетическими нажатиями в браузере; язык чипов брался из несуществующего в Lampa 3.3.4 `Lampa.Lang.code()` и в русском интерфейсе показывались английские названия — исправлено в Task 20 (`LC.langCode`).
...
> **ВЫПОЛНЕНО** (коммит `feat: финал фазы 2`): каталог опубликован на GitHub Pages и прописан в `LC.MANIFEST_URL` (`https://azarkostya.github.io/lumen-card/manifest.json`) — плагин тянет его сам, кэширует 12 ч, при недоступности сети берёт встроенный, при ответе, не проходящем `validate`, оставляет рабочий кэш (всё три случая проверены живьём). Настройки фазы собраны в группу «Главная и подборки» (профили настроения · персональные ряды · какие ряды показывать · количество рядов · скрывать досмотренное · свой каталог), добавлены кнопка-параметр `lumen_home_rows` с экраном выбора на `Lampa.Select` (147 подборок, отмеченные первыми, группы разделителями, запись на каждую галочку) и переключатель `lumen_moods`. Каждая настройка применяется на лету: ряды перерегистрируются и экран пересобирается сам (`LC.refreshComponent` → `Lampa.Activity.replace`), причём под открытым слоем настроек пересборка откладывается до выхода из него (иначе замена активности закрывает раздел и уводит фокус). Подсказка «Ключ API не задан» получила кнопку «Скрыть» и в карточке, и в сетке подборки Кинопоиска; вернуть — переключателем «Подсказка про ключ» рядом с полем ключа. Попутно исправлена локализация чипов настроения (Task 19). README переписан под фазу 2: что появилось, полная таблица настроек, раздел «Каталог подборок», честные ограничения (ничего не проверялось на реальном ТВ; ключ Кинопоиска лежит открытым текстом и уходит на сторонний домен; 500 запросов в сутки). Тестов 961 → 993, все зелёные; `es5check` чистый; живая проверка на Lampa 3.3.4 без ошибок в консоли. Долги фазы: герой ~36 % экрана вместо 58 %; в сетке подборки нет мини-героя; переход «вверх из первого ряда на чипы» не проверен физическим пультом. Ревью фазы запускает координатор отдельно.
...
## 6. Фаза 2 закрыта (2026-09-17)

> **ИТОГОВОЕ РЕВЬЮ ФАЗЫ** по снимку `dcefc26` → два Critical, закрыты в `045937c` («fix: находки итогового ревью фазы 2»). Обе находки координатор проверил лично по коду и вендору.
>
> **C1 — возврат на главную убивал недогруженные ряды.** Поколение рядов поднималось на `archive || destroy` компонента `main`, хотя `archive` в 3.3.4 означает «снова на экране», а не «ушли в фон» (это разобрано в `src/90_runtime.js:341-356` со ссылкой на `backward()`, и там же написано верно — а в `44_rows.js` комментарий утверждал обратное). Цена: Lampa ждёт, пока КАЖДАЯ часть пачки (`parts_limit = 6`) вызовет свой `call`, а мёртвый ряд не звал его никогда (`notifySubs` пропускает мёртвых подписчиков, ветка `no_sources` молчит, `pending--` в персональных рядах стоял под `if (!alive()) return`). Пачка не завершалась → главная переставала достраиваться до перезапуска Lampa. Исправлено: отмена только по `destroy`; введён `makeResolver` — call-функция ряда резолвится ровно один раз при любом исходе, `bumpGen()` закрывает незавершённые ряды пустым результатом. Живьём: уход в карточку в момент догрузки пачки и возврат — рядов стало 9, далее 11 и 13 по докрутке.
>
> **C2 — пересборка главной поверх открытого слоя уносила фокус.** `replaceSoon` проверял только имя активного компонента, а настройки и селектбокс — слой поверх активности (активностью всё это время остаётся главная); `type:'select'` пишет Storage до возврата контроллера, экран выбора рядов — на каждую галочку. Итог: `Activity.replace()` → `ActivitySlide.start()` → `Controller.toggle('content')`, и открытый список настроек переставал слушаться пульта. Исправлено: `layerOpen()` по тем же признакам, что использует сама Lampa (`body.settings--open`, `body.selectbox--open`, `Lampa.Select.opened()`, наличие `.modal`), проверка ВНУТРИ таймера, откладывание через `pending_refresh` + `Settings.listener 'close'`. Живьём подтверждено, что настройки — слой (`Activity.active().component === 'main'` при `Controller.enabled().name === 'select'`): три переключения галочек дали 0 вызовов `replace`, после закрытия настроек — 1 и корректная пересборка.
>
> Там же закрыты: чипы настроения читали несуществующий `LC.manifest.current()` и игнорировали пользовательский каталог; вторая подписка на `activity` в `49_moods.js` (чипы монтируются из общей точки после героя); переиспользование id подписчика в `43_sources.js` могло поздним `clear()` убить чужой новый запрос с тем же ключом (сквозной счётчик + сверка ссылки на запись); два комментария приведены к правде (ленивость `ContentRows` и гард героя).
>
> **Состояние на закрытие фазы:** 1011 тестов зелёные, `dist` 348 КБ, `es5check` чистый, каталог из 147 подборок опубликован на GitHub Pages и подхватывается плагином, всё запушено в `feat/lumen-v2`.
>
> **Долги, перенесённые в фазу 3:**
> 1. Цена первого экрана главной: в первой же пачке до 12 параллельных запросов деталей сериалов («Новые серии»), и первый кадр ждёт самого медленного из них. Лечится снижением `SHOWS_LIMIT`, отдачей ряда по дедлайну или переносом персональных рядов за первую пачку.
> 2. Герой занимает ~36 % высоты экрана вместо 58 % по дизайну — карточки рядов Lampa (290×563) крупнее дизайнских (230×345).
> 3. В сетке подборки содержимое не прокручивается вместе с шапкой, поэтому мини-герой (Step 4) не включён.
> 4. Герой пересобирается на каждом возврате из карточки (его снимает `detach` при уходе вглубь) — лишнее декодирование кадра на слабом ТВ.
> 5. `picked`/`shows` в `45_personal.js` захватываются один раз за активацию: ряд «Потому что вы смотрели» до конца сессии показывает прежний фильм.
> 6. Переход «вверх из первого ряда на чипы настроения» не проверен физическим пультом.
> 7. Встроенный каталог (~27 КБ) дублирует хостинговый; сокращать решено НЕ было — выигрыш 6 % размера против пустой главной при первом запуске без сети.
> 8. Ничего из фазы не проверялось на реальных Tizen / webOS / Android TV.
>
> **Статус долгов на 2026-09-23:** (2) снят Task 36 (фаза 4): высота героя — доли экрана, две трети в покое (`HERO_VH`, `src/30_css.js`); (3) — сознательно: мини-героя в сетке нет, довод в README («Ограничения», абзац про героя); (4) **закрыт `b99531c`** — уход вглубь паркует героя, возврат будит его с теми же узлами и кадром, слайдшоу «Несколько кадров» продолжает счёт; (5) **закрыт `4cb66b5`**; (7) — решено не делать (там же). Открыты и ждут телевизора: (1), (6), (8) — `docs/plans/2026-09-22-lumen-final.md`, «Что остаётся открытым».
