...
> **ВЫПОЛНЕНО** (33252a9, e655bd4, feb4cc1, a0b67e2, d1dcae9, d1bbf9c): `LC.torrents` в `src/65_torrents.js` — все правила пути в `<style id="lumen-torrents-css">` + `body.lumen-torrents-on`, `scoped()` с маркерами из `LC.menus`, класс `lumen-torrents` на активности, маски иконок дедуплицированы (одно правило на иконку); `LC.tokens` в `30_css.js`; `lumen-motion-*` на body; иконки `search`/`check`; одна пересборка CSS на смену настройки (`onChange` — фолбэк при провале подписки на Storage). Осознанно не сделано (нужна разметка): подписи «Подключаемся к TorrServer» / «Назад — отменить», текст «ПРОДОЛЖИТЬ», приглушение просмотренных серий, замена спрайтов Lampa маской в Select (без `:has()`). Пилюля статуса предзагрузки светлее экспорта ради читаемости. 324 теста, стенд 31/31, хэши 7/7. Долг ревью (надёжность `onChange` при падении чужого подписчика Storage, тесты) — перенесён в Task 10.
...
> **ВЫПОЛНЕНО**: Step 1 — сверка размеров экранов 33–40 со спецификацией (verifier, PASS; расхождения экрана 40 — сложение em, не дефект); Step 2 — живой регресс (qa, 9/10 PASS + дефекты пилюли статуса исправлены в d1bbf9c; путь идентичен без плагина, «Мои торренты», настройки в режимах all/path, переключение без перезагрузки, чужие иконки, хэши 7/7, 1280×720 и 4K, консоль); Step 3 — ревью качества фазы (Approved with minor ×2); Step 4 — README (69f4acc).
