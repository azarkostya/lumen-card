...
**Порядок:** исполняется после Task 11 фазы 1 и до Task 12–13 (ревью и публикация), чтобы первый релиз включал весь путь до плеера. Блоки «Поправки контроллера» приоритетнее текста задач.
...
| Экспорт дизайна (когда пользователь положит) | `design/*.dc.html`, экраны по комментариям `<!-- 33 --- -->` … `<!-- 40 --- -->`; главный для кода — FHD |
| Токены, шрифты, иконки | план фазы 1 §0.4, `src/20_icons.js` |

Если экспорта экранов 33–40 ещё нет, Task 32 делается по брифу и токенам карточки (размеры — как у аналогичных элементов карточки: чипы, кнопки, ряд серий), а после появления экспорта — сверка отдельным шагом (Task 33 Step 1).
