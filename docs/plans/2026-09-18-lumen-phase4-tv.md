...
- [x] **Step 4: Тесты зелёные, живая проверка** (скриншоты до/после «Крутить»). **Commit** `design: рулетка — спокойный экран, результат открывается кадром (Task 44)`.

**ВЫПОЛНЕНО 2026-09-21 (`863f79d` + фикс-раунд `37c24f9`), запушено в `feat/lumen-v2`, jsDelivr сброшен. Тесты 1721/1721, сборка актуальна, ES5 ok.**

Что в наброске выше оказалось неверным (исправлено при реализации):

- **Слой перехода не может жить до «Крутить ещё».** Карточка результата лежит внутри прокрутки, у `.scroll__body` стоит `will-change:transform` (`vendor/lampa/css/app.css:2762-2769`), у `.wrap__content` тоже (`:1044`) — собственный контекст наложения, никакой `z-index` изнутри не поднимется выше слоя в `<body>`. Поэтому кадр передаётся фону, карточка рисуется под ним, слой снимается — три вызова в одном кадре отрисовки.
- **`show(source)` без изменений не годится** — он растворяет слой в ноль, «держится до `stop()`» означало бы «держится невидимым». Понадобился флаг `hold`, снимающий растворение, `listen()` и таймер `LIFE`.
- **`live.done` в наброске нигде не выставлялся** — таймер звал `then` вторым разом; `one('transitionend')` ловил бы первое событие любого свойства, а префиксный `webkitTransitionEnd` не слушался вовсе.
- **`18em × 27em` не влезают:** корню рулетки остаётся 440 px из 540 (`.scroll` 497 минус два поля маски по 28.5), 27em высоты дают 467. Взято `43vh`. Собственное нижнее поле корня `2.81em` в первой редакции тоже не учли — на двух верхних ступенях масштаба оно давало ненулевой диапазон прокрутки и рывок экрана на ~38 px при заходе фокуса на «Крутить»; поле снято, замер по всем двенадцати сочетаниям («мельче/обычный/крупнее» × четыре `lumen_scale`) даёт `scrollHeight === высота области`, запас под подсказкой не меньше 18 px.

Дефекты, найденные ревью и закрытые фикс-раундом:

- `enterKadr()` не переустанавливал коллекцию фокуса. В режиме «Лёгкие» это **100 % случаев**: барабан не крутится, `showResult` зовётся синхронно за `prepareFrame`, `frame.ready` всегда false (onload асинхронен), значит `paintResult`/`recollect` отрабатывают при `kadr === false`, и «вверх» с кнопки результата уводило фокус на погашенный `opacity:0` барабан.
- Режим кадра с пустой карточкой: `result` выставлялся до колбэка, а `LC.transition.stop()` из `src/90_runtime.js:483` (старт любой не-`full` активности) колбэк отменял. Уход в левое меню в окне удержания и возврат назад давали кадр во весь экран без единого фокусируемого узла.
- После «Крутить ещё» коллекция не переустанавливалась — на 3.1 с разгона барабана часть кнопок пульта не отвечала.
- Лента чипов не сбрасывалась при смене «Фильмы/Сериалы» (`chipsScroll.reset()`).

Живая проверка координатора на стенде 960×540@2: барабан 155×232 по центру, низ подсказки 453 при кромке 540; чипы подборок — одна строка из 15 штук, последний уезжает за кромку и лента прокручивается сдвигом (`translate3d(-662px,0,0)`); все двенадцать сочетаний размера интерфейса и масштаба дают нулевой диапазон прокрутки; после переключения на «Сериалы» лента сброшена в 0.
...
- **ЗАКРЫТ 2026-09-23 решением «оставить» (без правки кода).** ~~Режим «Герой: выключен» остаётся со штатной маской Lampa на движущейся области (класс `lumen-main` там не ставится). Перепроверено живьём в `ebde4bf` (2026-09-23) и оставлено открытым: на главной без героя `.scroll.layer--wheight` несёт `mask-image` 0/8/92/100 % (`vendor/lampa/css/app.css:2781-2786`); снять маску — значит заменить затухание своим оверлеем, а это решение о виде режима, которым пользователь не пользуется.~~ Довод. Маска там не остаток, а часть штатного вида главной Lampa, в которую этот режим и превращает экран: под ней лежит работающий штатный фон. Стенд 960×540, DPR 2, кегль 11.4055, «Кадр над рядами: выключен», фокус во втором ряду: героя нет, `body.lumen-main-on` не стоит (метку ставит только смонтированный герой, `src/48_hero.js`, `markBody`), обёртка `Background.change` не стоит — перевод фокуса на соседнюю карточку дошёл до `Lampa.Background.change` с постером `w300` (1 вызов), `.background` Lampa — `position:fixed`, `display:block`, `opacity:1`, у области рядов `mask-image` 0/8/92/100 %. То есть ряды затухают к кромкам в фон с постером карточки под фокусом — ровно так, как в Lampa без плагина. Своё затухание (оверлеи Task 38) рисует цветом страницы и живёт под `.lumen-main` — поверх штатного фона оно было бы заплаткой. И выключить маску можно штатно: у Lampa есть свой пункт «Затухание» (Настройки → Интерфейс, `data-name="mask"`, по умолчанию вкл, `app.min.js:47906`; выключение ставит `body.no--mask`, `app.min.js:31771`, и снимает маску `app.css:273-276`) — цена маски на движущемся слое остаётся выбором пользователя, а не нашим.
- Горизонтальные маски рядов (`.scroll--horizontal.scroll--mask`, `app.css:2798`) — по одной на ряд, на движущихся слоях.
- `lumen_scale` не действует на текст и логотип героя: `SCALE_ROOTS{font-size}` (`30_css.js:605`) и `.lumen-hero .lumen-hero__text{font-size:1.1em}` (`:1746`) имеют одинаковую специфичность, второе ниже по файлу. **Статус 2026-09-23: так устроено намеренно** (с `b38425a`: комментарий у `LC.uiScale` в `src/30_css.js` и `LOGO_EM` в `src/48_hero.js` — логотип считается с масштабом 1); распространить масштаб плагина на героя — вопрос вида, за пользователем.
- **ЗАКРЫТ `ebde4bf` (2026-09-23).** ~~`LC.hud.layers()` не считает два новых принудительных слоя (он считает рисующие узлы, а это контейнеры) — цифра в HUD занижена на 2.~~ Счёт включает корень героя и область рядов; замер на стенде 960×540, главная: 7 → 9.
...
- **ЗАКРЫТ `6c4becd` (A2, волна A).** ~~Переключение настройки «Метки на постерах» на уже нарисованном экране показывает рейтинг дважды до следующей отрисовки: `strip()` снимает метки и полосы, но `.card__age` не правит.~~ `strip()` возвращает подписи прежний текст (`unrate`, `src/62_badges.js`); с `d0738c0` это проверено на нескольких карточках сразу.
...
### Б5 · закрыт фактом: HUD на ТВ пользователя — `960×540@2`
CSS-окно WebView 960×540, `devicePixelRatio = 2` → растр 1920×1080 физических пикселей. Это потолок WebView на этом ТВ: интерфейс рисуется в честном 1080p, до 4K растягивает сам телевизор; 4K через Lampa невозможен. Следствия для кода:
- база em Lampa: `max(960 / 84.17, 10.6) = 11.41` CSS px = 22.8 физических — те же размеры, что на стенде 1920@1;
- `LC.util.screenPx()` = 960 × 2 = 1920 → `frameSize()` отдаёт `original` — то есть Б1 п. 1 на этом ТВ действует точно;
- все `1px` в нашем CSS и в Lampa на экране — 2 физических пикселя;
- стенд `vendor/lampa` для приёмки надо гонять в режиме 960×540 @ DPR 2, а не 1920×1080 @ DPR 1 — геометрия в CSS-пикселях отличается вдвое, и пороги в px (если такие есть) срабатывают иначе.
