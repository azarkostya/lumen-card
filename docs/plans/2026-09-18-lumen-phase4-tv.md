...
- Режим «Герой: выключен» остаётся со штатной маской Lampa на движущейся области (класс `lumen-main` там не ставится). **Перепроверено живьём в `ebde4bf` (2026-09-23) и оставлено открытым:** на главной без героя `.scroll.layer--wheight` несёт `mask-image` 0/8/92/100 % (`vendor/lampa/css/app.css:2781-2786`); снять маску — значит заменить затухание своим оверлеем, а это решение о виде режима, которым пользователь не пользуется.
