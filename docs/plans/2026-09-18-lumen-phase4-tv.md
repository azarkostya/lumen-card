...
**Запушено в `feat/lumen-v2`, jsDelivr сброшен. Тесты 1561/1561, сборка актуальна, ES5 ok.**

### Закрыто и проверено живьём на стенде
Волна 0: Task 31 (HUD). Волна 1: Task 32-35. Волна 2: Task 36-40. Все с ревью и фикс-раундами.
...
### Осталось (волна 3 + калибровка)
- **Task 42 · карточки рядов**: убрать кольцо фокуса (`:after` Lampa) и пёстрые бейджи с постеров, фокус = `scale(1.08)` + тень, подпись «2017 · ★ 6.4» в `.card__age`, заголовки рядов 1.5em.
- **Task 43 · типографика**: один шрифт вместо трёх (убрать Unbounded и моноширинный), рамки у чипов/рейтинга/статуса/кнопок → 0, рейтинг героя в строку меты.
- **Task 44 · рулетка**: спокойный экран до результата, барабан по центру, результат открывается кадром через новый публичный `LC.transition.reveal(source, opts)`.
