/*!
 * Lumen Card — плагин перерисовки карточки фильма/сериала для Lampa
 * Строгий ES5, без внешних зависимостей (кроме jQuery, который уже есть в Lampa).
 * Совместимость: Smart TV webkit 2016+, Android 4.1 webview.
 */
(function () {
    'use strict';

    if (window.lumen_card_plugin) return;
    ...
    function warn(msg, err) {
        try { if (window.console && console.log) console.log('[lumen-card] ' + msg, err || ''); } catch (e) { }
    }
    ...
    function theme() {
        var key = pref(PLUGIN + '_accent', 'sand');
        return ACCENTS[key] || ACCENTS.sand;
    }

    function useFonts() {
        return pref(PLUGIN + '_fonts', true);
    }
    ...
    function seasonsWord(n) {
        if (isSlavic()) return plural(n, ['сезон', 'сезона', 'сезонов']);
        return n === 1 ? 'season' : 'seasons';
    }

    function episodesWord(n) {
        if (isSlavic()) return plural(n, ['серия', 'серии', 'серий']);
    ...
    /* Секунды -> "01:12" (часы:минуты) или "18:40" (минуты:секунды) */
    ...
    function movieProgress(movie) {
        ...
        if (!key) return null;
        var view = timelineView(utilsHash(key));
        if (view && view.percent > 0) return { view: view, season: 0, episode: 0 };
        return null;
    }

    function serialProgress(movie) {
        ...
        for (var s = 1; s <= maxSeason; s++) {
            for (var ep = 1; ep <= 30; ep++) {
                var hash = utilsHash([s, s > 10 ? ':' : '', ep, key].join(''));
                var view = timelineView(hash);
                if (view && view.percent > 0) {
                    if (!best || (view.updated || 0) >= (best.view.updated || 0)) {
                        best = { view: view, season: s, episode: ep };
                    }
                }
            }
        }
    ...
    function renderMeta(root, movie) {
        ...
        if (countries.length) parts.push('<span>' + esc(countries.slice(0, 2).join(' · ')) + '</span>');
        ...
        if (serial) {
            ...
            if (counts.length) parts.push('<span>' + esc(counts.join(' · ')) + '</span>');
        } else if (movie.runtime > 0) {
            parts.push('<span>' + esc(fmtRuntime(movie.runtime)) + '</span>');
        }

        var quality = !movie.first_air_date ? (movie.release_quality || movie.quality) : '';
        if (quality) parts.push('<span class="lumen-meta__chip">' + esc(('' + quality).toUpperCase()) + '</span>');
        else if (serial) parts.push('<span class="lumen-meta__chip">' + esc(lang('lumen_card_serial')) + '</span>');

        var genres = getGenres(movie);
        if (genres.length) parts.push('<span>' + esc(genres.join(', ')) + '</span>');

        var pg = getPG(movie, root);
        if (pg) parts.push('<span>' + esc(pg) + '</span>');
    ...
    function renderProgress(root, movie) {
        ...
        row.addClass('hide');
        if (!pref(PLUGIN + '_progress', true)) return;

        var found = isSerial(movie) ? serialProgress(movie) : movieProgress(movie);
        if (!found || !found.view || !(found.view.percent > 0)) return;

        var percent = Math.max(0, Math.min(100, Math.round(found.view.percent)));
        var label = lang('lumen_card_continue');
        if (found.season) label += ' · S' + found.season + ' E' + found.episode;

        var time = '';
        if (found.view.duration > 0) time = fmtClock(found.view.time) + ' / ' + fmtClock(found.view.duration);
        else if (found.view.time > 0) time = fmtClock(found.view.time);
        else time = percent + '%';
    ...
    function renderCast(root, data) {
        ...
        if (!pref(PLUGIN + '_cast', true)) return;
        ...
        for (var i = 0; i < limit; i++) {
            html.push('<div class="lumen-cast__item">' + esc(initials(cast[i] && cast[i].name)) + '</div>');
        }
        if (cast.length > limit) {
            html.push('<div class="lumen-cast__item lumen-cast__more">+' + (cast.length - limit) + '</div>');
        }

        block.find('.lumen-cast__label').text(lang('lumen_card_cast'));
    ...
    function addSettings() {
        try {
            ...
            Lampa.SettingsApi.addComponent({
                component: PLUGIN,
                icon: ICON,
                name: lang('lumen_card_title')
            });

            var accentValues = {
                sand: lang('lumen_card_accent_sand'),
                ice: lang('lumen_card_accent_ice'),
                wine: lang('lumen_card_accent_wine'),
                mint: lang('lumen_card_accent_mint')
            };

            Lampa.SettingsApi.addParam({
                component: PLUGIN,
                param: { name: PLUGIN + '_accent', type: 'select', values: accentValues, 'default': 'sand' },
                field: { name: lang('lumen_card_accent') },
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: { name: lang('lumen_card_fonts_name'), description: lang('lumen_card_fonts_descr') },
                onChange: function () { injectFonts(); injectCss(); }
            });

            Lampa.SettingsApi.addParam({
                component: PLUGIN,
                param: { name: PLUGIN + '_progress', type: 'trigger', 'default': true },
                field: { name: lang('lumen_card_progress_name') }
            });

            Lampa.SettingsApi.addParam({
                component: PLUGIN,
                param: { name: PLUGIN + '_cast', type: 'trigger', 'default': true },
                field: { name: lang('lumen_card_cast_name') }
            });
        } catch (e) {
    ...
    function followStorage() {
        try {
            ...
            Lampa.Storage.listener.follow('change', function (e) {
                ...
                if (e.name === PLUGIN + '_fonts') injectFonts();
                injectCss();
            });
        } catch (err) {
    ...
    function init() {
        try {
            ...
            try { if (Lampa.Lang && typeof Lampa.Lang.add === 'function') Lampa.Lang.add(STRINGS); } catch (e) { }
            ...
            try {
                Lampa.Template.add('full_start_new', buildTemplate());
            } catch (e) {
