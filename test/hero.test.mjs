...
/* Волна 3 (ТВ 2026-09-24, фото 15/16/17): кадр героя — во весь экран и
   ...
   нигде.
   Ревью раунда хвостов, п.7: левое затемнение — ПОД верхним/нижним.
   Затемнения красятся тенью подкраски (темнее фона страницы), и сплошной
   низ покоя цвета фона рядов обязан лежать поверх левого — иначе оно
   затемняло бы фон рядов слева (сторож — test/css.test.mjs, «фон под
   рядами — подкраска как есть»). */
...
/* Раунд правок финальной проверки, A1 (ревью rv2, RV2-1): у цвета показа
   тот же потолок HOLD_MS от вывода текста, что у кадра прошлого фильма, и
...
test('A1: экран уже нейтральный — цвет следующего фильма через HOLD_MS от вывода текста, одна запись', () => {
  ...
  answerDetails(env);
  env.advance(HOLD + 10);
  assert.equal(activeSrc(main), undefined, 'предусловие: кадр второго не успел — нейтральный фон');
  assert.deepEqual(env.calls, [11, 22], 'предусловие: цвет второго — с нейтральным фоном');
  rest(env);
  main.card2.removeClass('focus');
  card3.addClass('focus');
  fireFocus(main.activity, card3);
  ...
  answerDetails(env);
  env.advance(HOLD - 20);
  assert.deepEqual(env.calls, [11, 22], 'до потолка цвет ждёт кадра');
  env.advance(30);
  assert.deepEqual(env.calls, [11, 22, 33], 'кадр не встал за HOLD_MS — цвет третьего фильма');
  frameImg(env, '/b33.jpg').onload();
  assert.equal(activeSrc(main), 'https://img/t/p/w1280/b33.jpg', 'предусловие: кадр доехал позже');
  env.advance(9000);
  assert.deepEqual(env.calls, [11, 22, 33], 'поздний кадр цвет не повторяет');
});
...
test('A1: на экране только подложки, полный кадр едет — цвет через HOLD_MS от возврата фокуса, одна запись', () => {
  ...
  assert.equal(lqip.attr('src'), 'https://img/t/p/w300/b1.jpg', 'предусловие: подложка первого, полный кадр едет');
  env.advance(HOLD + 10);
  assert.deepEqual(env.calls, [], 'первый показ ждёт кадра');
  rest(env);
  main.card1.removeClass('focus');
  main.card2.addClass('focus');
  fireFocus(main.activity, main.card2);
  ...
  env.advance(SWAP + 10);
  answerDetails(env);
  assert.equal(lqip.attr('src'), 'https://img/t/p/w300/b2.jpg', 'предусловие: подложка второго встала при уходе фокуса');
  card3.removeClass('focus');
  main.card2.addClass('focus');
  fireFocus(main.activity, main.card2);
  env.advance(HOLD - 20);
  assert.deepEqual(env.calls, [], 'до потолка цвет ждёт кадра');
  env.advance(30);
  assert.deepEqual(env.calls, [22], 'кадр не встал за HOLD_MS — цвет второго фильма');
  frameImg(env, '/b2.jpg').onload();
  env.advance(9000);
  assert.deepEqual(env.calls, [22], 'одна запись');
});

/* Раунд правок финальной проверки, A2 (ревью rv2, RV2-2): сторожа
   отложенного цвета — возврат фокуса на показанную карточку (onFocus) и
   парковка (flushAccent). */
test('п.5: кадр встал, пока фокус на соседе; фокус вернулся — цвет показанного фильма ставится', () => {
  const env = accentEnv({ fxHeavy: () => false });
  const main = makeMain();
  const card3 = addCard(main, 33);
  env.hero.mount(main.activity);
  ...
  env.advance(50);
  late.onload();
  assert.deepEqual(env.calls, [11], 'фокус ушёл — цвета нет');
  env.advance(50);
  card3.removeClass('focus');
  main.card2.addClass('focus');
  fireFocus(main.activity, main.card2);
  assert.deepEqual(env.calls, [11, 22], 'фокус вернулся на показанный фильм — его цвет');
  env.advance(5000);
  assert.deepEqual(env.calls, [11, 22], 'одна запись');
});

test('п.5: парковка, пока кадр едет; «Выкл» из карточки — цвет главной под карточкой не ставится', () => {
  const mode = { v: 'full' };
  const env = accentEnv({ motionMode: () => mode.v });
  const main = makeMain();
  env.hero.mount(main.activity);
  ...
  mode.v = 'off';
  env.hero.applyMotion();
  assert.deepEqual(env.calls, [11], 'под карточкой цвет главной не ставится');
  env.hero.mount(main.activity);
  env.hero.accentBack();
  env.advance(SWAP + 2000);
  assert.equal(env.calls[env.calls.length - 1], 22, 'после возврата — цвет показанного фильма');
});

test('п.5: фокус ушёл до вывода текста и вернулся после кадра — цвет ставится', () => {
  const env = accentEnv({ fxHeavy: () => false });
  const main = makeMain();
  const card3 = addCard(main, 33);
  env.hero.mount(main.activity);
  ...
  fireFocus(main.activity, main.card2);
  env.advance(1000);
  assert.deepEqual(env.calls, [11, 22], 'фокус вернулся — цвет показанного фильма');
});

test('п.5: отказ кадра при уходе фокуса, возврат — нейтральный фон и цвет', () => {
  const env = accentEnv({ fxHeavy: () => false });
  const main = makeMain();
  const card3 = addCard(main, 33);
  env.hero.mount(main.activity);
...
/* Ревью раунда хвостов, п.6 (чек-лист: «через восемь секунд покоя» после
   ...
   а на закрытии «Назад» класс снят раньше (close -> hide, потом onBack),
   фокус возвращается на карточку внутри Controller.toggle(enabled), и
   только потом летит его 'toggle'. При выборе пункта порядок другой —
   класс снимается ПОСЛЕ 'toggle' (ревью правок раунда хвостов, п.1, тест
   ниже); проверку onToggle делает через такт, и тест её дожидается. */
...
/* Сторожа п.6. Уход без оверлея (шапка, левое меню — главную там видно)
   отсчёт не трогает: он идёт от первого фокуса. И меню, открытое поверх
   играющего ролика и закрытое «Назад», ролик не обрывает: отсчёта уже нет,
   а возврат фокуса на ту же карточку съедает гард «фокус не сменился».
   Ролик, стартовавший ПОСЛЕ выбора пункта меню, до ревью правок раунда
   хвостов обрывался первым же событием фокуса той же карточки — тест
   ниже. */
...
/* Ревью правок раунда хвостов, п.1: путь «выбор пункта меню». При выборе
   пункта Lampa сначала зовёт onBeforeClose меню — Controller.toggle(enabled)
   (app.min.js:22120-22123; наше меню — open в src/63_cardmenu.js), и
   только потом goclose -> hide снимает selectbox--open (:7027-7033).
   toggle возвращает фокус на карточку и лишь затем шлёт 'toggle'
   (:46297-46314): onToggle видит ещё не снятый класс. Прежде он забывал
   фокус второй раз, ролик через 8 с стартовал в «забытом» состоянии, и
   любое следующее событие фокуса той же карточки — пультом или мышью —
   проходило гард «фокус не сменился» и обрывало ролик. */
for (const input of ['пульт', 'мышь']) {
  test('трейлер героя (' + input + '): пункт меню выбран — ролик стартует через 8 с, и повторный фокус той же карточки его не обрывает', () => {
    const env = trailerEnv();
    const main = makeMain();
    const again = input === 'мышь' ? fireHover : fireFocus;
    env.hero.mount(main.activity);
    focusOn(main, main.card1);
    env.advance(3000);
    env.bodyClasses.push('selectbox--open');
    env.hero.onToggle();
    env.advance(2000);
    /* Выбор пункта: фокус вернулся и 'toggle' ушёл при открытом списке,
       класс снят следом — в том же такте. */
    fireFocus(main.activity, main.card1);
    env.hero.onToggle();
    env.bodyClasses.splice(env.bodyClasses.indexOf('selectbox--open'), 1);
    env.advance(7900);
    ...
    env.advance(200);
    assert.equal(videoCount(env), 1, 'после выбора пункта отсчёт не заведён');
    lastVideos(env).ok(VIDEOS_RU);
    env.players[0].onStart();

    again(main.activity, main.card1);
    assert.equal(env.players[0].destroys, 0, 'повторное событие фокуса той же карточки оборвало ролик');
    env.advance(3000);
    assert.equal(env.players[0].destroys, 0);
    assert.equal(videoCount(env), 1);
    assert.deepEqual(warnLog, []);
  });
}
...
/* Кадры с голосами TMDB (VOTES): их дало ревью правок волны 3, п.1, когда
   в смену кадров героя шли только годные для первого кадра. С ревью правок
   раунда хвостов, п.5, в смену идут и кадры без голосов (тесты ниже), и
   голоса здесь ничего не меняют. */
...
/* Ревью правок волны 3, п.1: ротация героя брала кадры через pickBackdrops —
   там отбор только по надписям, — и в круг попадали отвергнутые на TMDB
   кадры: у «Суперполицейских 3» шли ключевой арт 2.28 и кадры по 0.166.
   Ревью правок раунда хвостов, п.5 (решение координатора): правило 6a76db3
   (в круг — только кадры с голосами и не хуже половины оценки ключевого
   арта, как у первого кадра героя) оказалось слишком строгим — на выборке
   стенда смены кадров не было у 28 из 90 фильмов, а пользователь жаловался
   именно на «кадры не меняются». Правило разделено: первый кадр
   (heroBackdrop) — по-прежнему строго, а в круг идёт всё, кроме явно
   отвергнутых — с голосами и оценкой ниже 1 (кадр без голосов на TMDB —
   норма). Ключевой арт в круге — только если он и есть первый кадр. */
...
/* Круг смены кадров героя: кадр на экране и тики слайдшоу по одному. */
function heroCircle(env, main, first, ticks) {
  frameImg(env, first).onload();
  assert.equal(stageOf(heroOf(main.activity)).find('.lumen-hero__bg--a').attr('src'), 'https://img/t/p/w1280' + first, 'предусловие: первый кадр героя');
  if (!env.live().length) return null;
  const shown = [];
...
test('ревью правок волны 3, п.1: ключевой арт 2.28 и три кадра по 0.166 — смена кадров героя не заводится', () => {
  const env = slidesEnv();
  try {
    const main = makeMain();
    env.hero.mount(main.activity);
    ...
    detailsOf(env, 11).ok({ id: 11, backdrop_path: '/b1.jpg', images: { logos: [], backdrops: [
      voted('/b1.jpg', 2.28, 3), voted('/r0.jpg', 0.166, 1), voted('/r1.jpg', 0.166, 1), voted('/r2.jpg', 0.166, 2)
    ] } });
    assert.equal(heroCircle(env, main, '/b1.jpg', 0), null, 'смена кадров заведена по отвергнутым кадрам');
    assert.deepEqual(frameLoads(env), ['/b1.jpg'], 'грузились кадры кроме ключевого');
    assert.deepEqual(warnLog, []);
  } finally { env.restore(); }
});

/* Ревью правок раунда хвостов, п.5: до правки — «не заводится» (6a76db3):
   так на стенде стояли Zip Wire и «Операция „Возмездие“». */
test('ревью правок раунда хвостов, п.5: ни у ключевого арта, ни у кадров нет голосов — смена кадров героя заводится', () => {
  const env = slidesEnv();
  try {
    const main = makeMain();
    env.hero.mount(main.activity);
    ...
    env.advance(400);
    detailsOf(env, 11).ok({ id: 11, backdrop_path: '/b1.jpg', images: { logos: [], backdrops: [
      voted('/b1.jpg', 0, 0), voted('/r0.jpg', 0, 0), voted('/r1.jpg', 0, 0)
    ] } });
    assert.deepEqual(heroCircle(env, main, '/b1.jpg', 4), ['/r0.jpg', '/r1.jpg', '/b1.jpg', '/r0.jpg'],
      'кадры без голосов в смену не идут');
    assert.deepEqual(warnLog, []);
  } finally { env.restore(); }
});

/* Ревью правок раунда хвостов, п.5: голоса есть только у ключевого арта —
   первым кадром встаёт он (кандидату в кадр героя голоса нужны), а круг
   идёт по кадрам без голосов; у NAZA на стенде так — ключевой 3.334/1,
   кадр 0.166/1 и пять кадров без голосов. */
test('ревью правок раунда хвостов, п.5: голоса только у ключевого арта — круг по кадрам без голосов, отвергнутый пропущен', () => {
  const env = slidesEnv();
  try {
    const main = makeMain();
    env.hero.mount(main.activity);
    ...
    env.advance(400);
    detailsOf(env, 11).ok({ id: 11, backdrop_path: '/b1.jpg', images: { logos: [], backdrops: [
      voted('/b1.jpg', 3.334, 1), voted('/rejected.jpg', 0.166, 1), voted('/n0.jpg', 0, 0), voted('/n1.jpg', 0, 0)
    ] } });
    assert.deepEqual(heroCircle(env, main, '/b1.jpg', 3), ['/n0.jpg', '/n1.jpg', '/b1.jpg'],
      'в круге отвергнутый кадр или нет кадров без голосов');
    assert.deepEqual(warnLog, []);
  } finally { env.restore(); }
});

/* Ревью правок раунда хвостов, п.5: граница «явно отвергнут» — голоса есть
   и оценка ниже 1. Отвергнутые: 0.166 (все «против»), 0 при голосе, 0.958;
   остаются 1.222 и выше, кадры без голосов, и кадры ниже половины оценки
   ключевого арта, если они не отвергнуты. Порядок — pickBackdrops (широкие
   вперёд, по оценке), ключевого арта в круге нет: первым встал не он.
   В полном режиме — до 8 кадров, чтобы круг не резал потолок lite. */
test('ревью правок раунда хвостов, п.5: в круге героя всё, кроме явно отвергнутых (голоса и оценка ниже 1), и без ключевого арта', () => {
  const env = slidesEnv({ motion: 'full' });
  try {
    const main = makeMain();
    env.hero.mount(main.activity);
    focusOn(main, main.card1);
    ...
    detailsOf(env, 11).ok({ id: 11, backdrop_path: '/b1.jpg', images: { logos: [], backdrops: [
      ...
      voted('/zero.jpg', 0, 1), voted('/quarter.jpg', 0.958, 4), voted('/third.jpg', 1.222, 3),
      voted('/half.jpg', 2.65, 1), voted('/low.jpg', 2.6, 3)
    ] } });
    assert.deepEqual(heroCircle(env, main, '/good.jpg', 6), ['/half.jpg', '/low.jpg', '/third.jpg', '/novote.jpg', '/good.jpg', '/half.jpg'],
      'в круге отвергнутый кадр (0.166, 0 при голосе, 0.958) или ключевой арт, или нет кадров без голосов и ниже половины ключевого');
    assert.deepEqual(warnLog, []);
  } finally { env.restore(); }
});
...
/* ====================================================================== */
/* Ревью H1: запрос роликов, который Lampa отменила молча                  */
/* ====================================================================== */
...
test('ревью H1: запрос роликов без ответа — через 25 с следующий язык, ещё через 25 с «err req»; поздний ответ ролик не заводит', () => {
  ...
  ru.ok(VIDEOS_RU);
  en.ok(VIDEOS_RU);
  assert.equal(env.players.length, 0, 'поздний ответ завёл ролик');
  assert.deepEqual(notes, ['plan', 'err req#1']);
  assert.deepEqual(warnLog, []);
});

test('ревью H1: ответ роликов в срок снимает таймер срока; снятие героя — тоже', () => {
  const env = trailerEnv();
  const main = makeMain();
  env.hero.mount(main.activity);
  focusOn(main, main.card1);
