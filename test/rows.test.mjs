...
test('dedupeAcross: «Досмотреть» ниже подборки — начатый фильм уходит и из подборки выше', function () {
  var rows = [
    ...
    mkRow('Досмотреть', [3, 9], { lumen_personal: true, lumen_own: true }),
...
/* Ревью волны 4 (60): предварительный проход клал в окно карточки ВСЕХ
   личных рядов пачки — и ~20 рекомендаций «Потому что» (место 5) съедали
   подборку-лидера на месте 0 до «огрызка». Вперёд идут только карточки
   самого пользователя (lumen_own): начатые фильмы и его сериалы. */
test('dedupeAcross: «Потому что» ниже лидера его не чистит — вперёд только «Досмотреть» и «Новые серии»', function () {
  var rows = [
    mkRow('Лидер', [1, 2, 3, 4, 5, 6]),
    mkRow('Досмотреть', [50], { lumen_personal: true, lumen_own: true }),
    mkRow('Сейчас смотрят', [20, 21, 22, 23]),
    mkRow('Новые серии', [6, 60], { lumen_personal: true, lumen_own: true }),
    mkRow('Подборка', [40, 41, 42, 43, 44]),
    mkRow('Потому что', [1, 2, 3, 4, 41, 42, 43, 70], { lumen_personal: true })
  ];
  var out = R.dedupeAcross(rows, {}, 4, 6);
  assert.deepEqual(out.map(function (r) { return r.title; }),
    ['Лидер', 'Досмотреть', 'Сейчас смотрят', 'Новые серии', 'Подборка', 'Потому что'], 'лидер на месте');
  assert.deepEqual(idsOf(out[0]), [1, 2, 3, 4, 5], 'из лидера ушёл только сериал из «Новых серий»');
  assert.deepEqual(idsOf(out[4]), [40, 41, 42, 43, 44], 'подборку выше «Потому что» рекомендации не трогают');
