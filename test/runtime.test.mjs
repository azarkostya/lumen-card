...
/* Волна 2 (ТВ 2026-09-24, B): второй экран карточки — обычная прокрутка за
   фокусом пульта (src/85_header.js, bindDescr). Модулю ряда описания
   (onToggle, last) и Scroll карточки (update, колесо) нужны e.item и
   e.link — они приходят только на build с name 'description'. Узел ряда —
   тот же, что у таблицы «ПОДРОБНО» и отзывов. */
test('build description: bindDescr получает модуль ряда, его узел и компонент карточки', () => {
  ...
  assert.equal(seen[0][0], item, 'прокрутка к блоку после Lampa — через onToggle самого модуля ряда');
  assert.equal(seen[0][1], descrRows[0], 'узел ряда тот же, что у таблицы «ПОДРОБНО»');
  assert.equal(seen[0][2], link, 'Scroll прокрутки и колеса — у компонента карточки');
  assert.deepEqual(warnLog, []);
});
...
/* Ревью Task 36, находка К1. Смена настройки «Размер кадра» на живой главной
   перемонтирует героя, а вместе с ним решается, где чипы: с волны 3 при
   живом кадре их на главной нет, а без кадра («Выключен») они стоят своим
   узлом в корне активности — LC.moods.mount смотрит, есть ли в корне узел
   героя. Отсюда два требования к LC.applyHeroSizePref, и оба про порядок, а
   не про поведение: чипы обязаны монтироваться ПОСЛЕ работы с героем (до неё
   DOM ещё в старом состоянии) и в ОБЕИХ ветках, включая «Выключен» (при
   Task 36 она уходила в return раньше монтирования, и чипов на экране не
   было до перезахода на главную).
...
test('К1: applyHeroSizePref монтирует чипы после героя и в обеих ветках размера', () => {
  ...
  /* И ни один return не должен стоять между героем и чипами — иначе ветка
     «Выключен» снимет героя, не дав чипам встать своим узлом. */
...
test('Important 2: старт главной монтирует чипы, и строго ПОСЛЕ героя', () => {
  ...
  const main = makeActivityObj('main', false);
  /* Фиксируем порядок: герой должен успеть встать в корень — по его узлу
     LC.moods.mount решает, ставить ли чипы. */
  const order = [];
