...
test('каждая настройка применяется ровно один раз на Storage.set', () => {
  const expected = {
    ...
    /* Постеры: источник постера — тот же путь. Обложка берётся при СБОРКЕ
    ...
    /* Волна 4: начало главной — тот же путь, что у личных рядов: план
       главной перерегистрирует ряды, главная пересобирается. */
    lumen_home_start: ['personal'],
    ...
    /* Task 31 (фаза 4): HUD отладки — sync() сам решает, показать узел или
       снять его. */
    lumen_debug_hud: ['hud'],
    /* Task 40 (фаза 4): тумблер тяжёлых эффектов — класс lumen-fx-heavy на
       body и слой частиц переставляет LC.applyMotionMode. Ротация кадров
...
/* ====================================================================== */
/* A5 (волна A финального плана): описание плоского вида — по факту.       */
/*                                                                         */
/* Ревью Task 73 и сам исполнитель признали: в сетке и хабе плоский вид     */
/* снимает ровно три подложки, и все три лежат ПОД картинкой (сторож на     */
/* сам набор правил — в test/css.test.mjs). Описание же обещало там         */
...
/* Долг Minor фикс-раунда фазы 3 (docs/plans/2026-09-15-lumen-phase3-features.md:251)
   закрывался строкой про «Переход от постера» в описании «Кадра над рядами».
   Волна 2 (ТВ 2026-09-24, D3): переход удалён вместе с настройкой — описание
   о нём больше не говорит и не ссылается на строку, которой нет. Значение
   «Выключен» оно по-прежнему называет. */
test('«Кадр над рядами»: описание без удалённого «Перехода от постера», значение «Выключен» названо', () => {
  const env = setup();
  const S = env.LC.STRINGS;
  assert.equal(S.lumen_transition_name, undefined, 'строка удалённой настройки осталась в словаре');
  assert.equal(S.lumen_transition_descr, undefined);
  const quote = { ru: ['«', '»'], uk: ['«', '»'], en: ['"', '"'] };
  const gone = { ru: 'Переход от постера', en: 'Poster transition', uk: 'Перехід від постера' };
  for (const lang of ['ru', 'en', 'uk']) {
    const text = S.lumen_hero_size_descr[lang];
    assert.ok(text, lang + ': описания нет');
    assert.equal(text.indexOf(gone[lang]), -1, lang + ': описание говорит об удалённом переходе: ' + text);
