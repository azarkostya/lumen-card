...
/* Правило кромки главной (src/30_css.js, правило .lumen-main .items-line).
   ...
   «кадра нет»).
   Волна «хвосты героя», п.I: ветки «кадра нет» шире 2.44:1 тестом не
   покрывались (самые широкие окна были 21:9 — 2.37 и 2.39:1) — добавлены
   окна 2.5:1 и 2.7:1; в них есть клетки обеих веток (сторож — тест п.I
   ниже). */
const EDGE_WINDOWS = [
  [960, 540],
...
/* Ревью раунда главной (~85): полосы, где «Мельче» и «Крупнее» берут
   раскладку «Обычных», — на всех окнах правила кромки, а не только на двух. */
test('ревью раунда главной: правило кромки при «Мельче» и «Крупнее» плитках — на всех окнах', () => {
  const bad = [];
  for (const tile of ['small', 'large']) {
    for (const [W, H] of EDGE_WINDOWS) bad.push(...edgeViolations(W, H, false, tile));
  }
  assert.deepEqual(bad, []);
});

/* Ревью раунда главной `7b473f6..52d2b12` (~85): «Крупнее» было МЕЛЬЧЕ
   «Обычных» на окнах шире 16:9 (средний кадр, «обычный» интерфейс: 1840×960,
   1920×969, 2560×1300 — 8.57 / 9.52 / 8.88em). База «Крупнее» шире, и её
   порог узкой колонки наступает раньше, чем у «Обычных», — между двумя
   порогами «Крупнее» уже на узкой колонке, а «Обычные» ещё нет. Порядок
   «Мельче» ≤ «Обычные» ≤ «Крупнее» обязан держаться на каждом окне, при
   каждом размере кадра и интерфейса — по фактической ширине карточки,
   которую выберет каскад (модель rowLayout). */
test('ревью раунда главной: «Мельче» ≤ «Обычные» ≤ «Крупнее» на всех окнах, размерах кадра и интерфейса', () => {
  const bad = [];
  for (const [W, H] of EDGE_WINDOWS.concat([[2560, 1300]])) {
    for (const iface of ['small', 'normal', 'bigger']) {
      for (const size of ['large', 'medium', 'compact']) {
        const w = {};
        for (const tile of ['small', 'normal', 'large']) {
          const built = withStorage({ lumen_hero_size: size, interface_size: iface, lumen_tile_size: tile }, (LC) => LC.buildCss(), W);
          w[tile] = rowLayout(built, W, H, { interface: iface }).cardW;
        }
        if (!(w.small <= w.normal + 1e-6 && w.normal <= w.large + 1e-6)) {
          bad.push(W + '×' + H + ' ' + iface + '/' + size + ': ' + [w.small, w.normal, w.large].map((v) => v.toFixed(2)).join(' / '));
        }
      }
    }
  }
...
/* Волна «хвосты героя», п.I: окна 2.5:1 и 2.7:1 добавлены ради веток
   «кадра нет» шире 2.44:1 — без них правило кромки там не проверялось.
   Сторож того, что окна туда действительно попадают: в каждом есть клетки
   обеих веток (порог «кадра нет» — от 2.2 у компактного кадра на «крупнее»
   до 3.19 у крупного на «мельче»), и среди клеток без кадра — средний кадр
   на «обычном» (порог 2.44: у окон 21:9 выше, 2.37 и 2.39, кадр там ещё
   есть). */
test('правило кромки, п.I: окна 2.5:1 и 2.7:1 — клетки обеих веток, средний кадр без кадра', () => {
  for (const [W, H] of [[2400, 960], [2592, 960]]) {
    let on = 0;
    let off = 0;
    for (const iface of ['small', 'normal', 'bigger']) {
      for (const size of ['large', 'medium', 'compact']) {
        const built = withStorage({ lumen_scale: 'normal', lumen_hero_size: size, interface_size: iface }, (LC) => LC.buildCss(), W);
        const gone = W / H >= parseInt(/min-aspect-ratio:(\d+)\/100/.exec(heroOffMedia(built))[1], 10) / 100;
        if (gone) off++;
        else on++;
        if (iface === 'normal' && size === 'medium') assert.ok(gone, W + '×' + H + ': у среднего кадра на «обычном» кадр ещё есть');
      }
    }
    assert.ok(on > 0 && off > 0, W + '×' + H + ': клеток с кадром ' + on + ', без кадра ' + off);
  }
});
...
test('ревью п.3: проявление текста героя не отнимает пульс у скелетонов', () => {
  ...
  /* Волна производительности (C9): при зависшем запросе скелетон пульсировал
     вечно. Теперь 8,5 повтора (около 12 с): половина последнего повтора
     кончается на 50 % ключевых кадров, где opacity — 1, то есть ровно там,
     где стоит плашка без анимации, — конец не даёт скачка. */
  assert.equal(/infinite/.test(pulse), false, 'пульс скелетона снова бесконечный: ' + pulse);
  assert.ok(/animation:lumen-sk 1\.4s ease-in-out 8\.5(;|$)/.test(pulse), 'конечное число повторов: ' + pulse);
});
...
test('дизайн C: описание результата — и в стандартном виде', () => {
  const d = findDecl(css, (sel) => sel === '.lumen-roulette .lumen-roulette__rdescr');
  assert.ok(d, 'правила описания результата в стандартном виде нет');
  assert.ok(/-webkit-line-clamp:2/.test(d), 'описание не в две строки: ' + d);
  assert.ok(parseFloat(/font-size:([\d.]+)em/.exec(d)[1]) >= 1.01);
});
...
/* Сверка 2026-09-26: «Осталось 57 мин» в ряду «Досмотреть» — метке
   разрешены две строки, как метке ряда: обрезать её значило бы съесть
   минуты. */
test('сверка: метка «Осталось N мин» переносится на вторую строку, а не режется', () => {
