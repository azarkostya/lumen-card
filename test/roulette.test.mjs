...
/* П.2. Удержанный слой снимает не только наш колбэк: старт любого экрана
   зовёт LC.transition.stop() (src/90_runtime.js). Колбэк тогда
...
/* Пятый раунд, п.4. Показ выборки уже ушёл в сеть (loadPool из
   ...
   spin() снимал только таймер показа, а loadPool видел пустой пул и
   заводил второй полный набор вызовов LC.sources.fetch (в сети летящие
   запросы склеивает fetchAll, отвеченные берутся из кэша — лишними были
   второй сборщик и повторная обработка). Теперь «Крутить» присоединяется
   к сбору в полёте. */
...
/* Пятый раунд, п.5. «Назад» из рулетки, пока каталог в пути: backward()
   ...
   destroy() рулетки зовёт только через 200 мс (backward(), vendor/lampa/
   app.min.js:45935-45980, setTimeout — :45947-45954); pause() не зовётся, started остаётся true. Ответ каталога в
