...
test('Task 60: на главной цвет не прыгает — он едет шагами и доезжает за 1.6 с', () => {
  ...
  withDom(dom, () => {
    ...
    /* Волна производительности: шаг 200 мс, а не 100, — за путь узел
       подкраски (около 1,8 КБ правил) переписывается восемь раз, а не
       шестнадцать: каждая запись — пересчёт стилей и перерисовка
       полноэкранных градиентов. */
    assert.equal(ctx.state.rules - written, 8, 'перезаписей узла за путь');
  });
});
...
/* Волна производительности (жалоба с ТВ «всё ещё лагает всё», 2026-09-24).
   ...
   шестнадцати шагов. Перехода больше нет, шаг — 200 мс. */
test('волна perf: у подложки рядов нет CSS-перехода, шаг пути 200 мс, путь не короче 1.5 с', () => {
  ...
  withDom(dom, () => {
    const ctx = accentCtx({ prefs: {} });
    assert.equal(ctx.LC.accent.timing().step, 200);
