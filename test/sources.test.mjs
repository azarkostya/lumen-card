...
/* ====================================================================== */
/* Постеры: источник постера карточки — чистая часть.                     */
/* ====================================================================== */
...
test('Постеры: допуск пропорции — 0.64…0.75, выброс 0.486 отсекается', () => {
...
/* Срез, который допуск разрешает, — в тех же числах, что и его границы.
   ...
   голова. */
test('Постеры: границы допуска — это 4 % высоты и 11.1 % ширины', () => {
  const box = 2 / 3;
...
test('Постеры: берётся первый постер без языка подходящей пропорции', () => {
  const json = {
    posters: [
...
test('Постеры: «без языка» — это строго null, а не «xx» и не пустая строка', () => {
...
test('Постеры: подходящего постера нет — пустая строка, карточка остаётся с постером Lampa', () => {
...
/* posterIndex / applyPosters — сопоставление списков в режиме «Английские» ('original'). */
test('Постеры: сопоставление по id учитывает медиа — фильм и сериал с одним id не путаются', () => {
...
test('Постеры: постера в карте нет — карточка остаётся со своим', () => {
  const map = S.posterIndex([
...
test('Постеры: тот же самый постер за подмену не считается', () => {
  const cards = [{ id: 1, title: 'A', poster_path: '/same.jpg' }];
  assert.equal(S.applyPosters(cards, { 'movie:1': '/same.jpg' }), 0);
});

test('Постеры: дырки в списке карточек ничего не роняют', () => {
