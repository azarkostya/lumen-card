...
/* Ревью раунда хвостов, п.1 (регрессия f3cb272 + 97185b9): карточки
   ...
   Оверлей, открытый при запросе, чужим не считается, пока он открыт
   (закрыли до ответа — тест ниже). */
...
/* Ревью правок раунда хвостов, п.2 (решение координатора): поиск, открытый
   при запросе, закрыли до ответа — человек нажал «Назад», то есть ушёл с
   экрана, где просил трейлер. Ответ чужой: ни плеера, ни уведомления — и
   вовремя, и с опозданием. До этой правки такой трейлер играл: оверлей,
   закрытый после запроса, чужим не считался (99d4850). */
test('трейлер из меню: запрос из поиска, поиск закрыли до ответа — не играет и молчит', () => {
  for (const late of [false, true]) {
    const env = trailerEnv();
    try {
      openSearch(env);
