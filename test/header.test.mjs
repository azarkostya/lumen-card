...
/* ------------------------- второй экран: страницы ------------------------- */

/* Правило кромки для второго экрана (src/85_header.js, bindDescr): ряд
   описания показывается страницами — основная, отзывы, «Смотреть по
   порядку». Модель DOM здесь своя, маленькая: код страниц ходит по живому
   DOM (parentNode, children, classList, querySelectorAll), а фейковый
   FakeEl из _fakedom.mjs этого не умеет. */
function TNode(cls, rect, kids) {
  ...
  (kids || []).forEach((k) => self.append(k));
  this[0] = this;
  this.length = 1;
}
TNode.prototype.append = function (k) { k.parentNode = this; this.children.push(k); };
TNode.prototype.getBoundingClientRect = function () { return this._rect; };
TNode.prototype.querySelectorAll = function (sel) {
  const cls = sel.replace(/^\./, '');
  const out = [];
  (function walkT(n) { n.children.forEach((c) => { if (c.classList.contains(cls)) out.push(c); walkT(c); }); })(this);
  return out;
};
TNode.prototype.addEventListener = function (type, fn, capture) { this._listeners.push({ type, fn, capture }); };

function pagesRow(opts) {
  opts = opts || {};
  const r = (left, top, width, height) => ({ left, top, width, height });
  const text = new TNode('full-descr__text selector', r(42, 78, 515, 188));
  const tagA = new TNode('tag-count selector', r(42, 290, 90, 35));
  const tagB = new TNode('tag-count selector', r(140, 290, 120, 35));
  const left = new TNode('full-descr__left', r(42, 78, 515, 267), [text, new TNode('full-descr__tags', r(36, 284, 526, 47), [tagA, tagB])]);
  const facts = new TNode('lumen-facts', r(42, 355, 876, 19));
  const hide = new TNode('lumen-reviews__hint-hide selector', r(65, 538, 61, 29));
  const reviews = opts.skeleton ?
    new TNode('lumen-reviews lumen-reviews--sk', r(42, 394, 876, 191), [new TNode('lumen-review lumen-review--sk', r(42, 394, 200, 150))]) :
    new TNode('lumen-reviews lumen-reviews--hint', r(42, 394, 876, 191), [hide]);
  const frMode = new TNode('lumen-fr__mode selector', r(650, 610, 60, 22));
  const frCard = new TNode('lumen-fr-card selector', r(42, 652, 95, 227));
  const frCard2 = new TNode('lumen-fr-card selector', r(147, 652, 95, 227));
  const fr = new TNode('lumen-fr', r(42, 606, 876, 275), [frMode, frCard, frCard2]);
  const kids = [left, facts, reviews];
  if (!opts.noFr) kids.push(fr);
  const holder = new TNode('full-descr', r(0, 78, 960, 804), kids);
  const row = new TNode('items-line lumen-descr-row', r(0, 78, 960, 840), [new TNode('items-line__body', r(0, 78, 960, 804), [holder])]);
  return { row, holder, text, tagA, tagB, left, facts, reviews, hide, fr, frMode, frCard, frCard2 };
}
...
function descrItem() {
  ...
  return {
    ...
    controller() {
      const calls = [];
      const ctrl = { down() { calls.push('lampa-down'); }, up() { calls.push('lampa-up'); } };
      comps.forEach((m) => { if (m.onController) m.onController(ctrl); });
      return { ctrl, calls };
    }
  };
...
/* Navigator и Controller в объёме, который трогают страницы: фокус,
   canmove по списку разрешённых направлений и collectionFocus, который, как
   у Lampa, не ставит фокус на узел со скрытой страницы — здесь «скрыт»
   значит лежит в блоке-странице без класса --on или в основной странице
   при классе --sub на ряду. */
function withPagesNav(c, focused, canDirs, fn) {
  const prevNav = window.Navigator;
  const prevCtl = Lampa.Controller;
  const visible = (el) => {
    for (let n = el; n && n !== c.holder; n = n.parentNode) {
      if (n.parentNode === c.holder) {
        const page = n.classList.contains('lumen-reviews') || n.classList.contains('lumen-fr');
        return page ? n.classList.contains('lumen-descr-page--on') : !c.row.classList.contains('lumen-descr-row--sub');
      }
    }
    return false;
  };
  const nav = {
    focus: focused,
    getFocusedElement() { return nav.focus; },
    canmove(dir) { return canDirs.indexOf(dir) !== -1 ? {} : false; }
  };
  const focusCalls = [];
  window.Navigator = nav;
  Lampa.Controller = {
    enabled: () => ({ name: 'full_descr', controller: { link: c.item } }),
    collectionFocus(target, html) {
      focusCalls.push([target, html]);
      if (target && visible(target)) {
        nav.focus = target;
        c.holder._listeners.filter((l) => l.type === 'hover:focus').forEach((l) => l.fn({ type: 'hover:focus', target: target }));
      }
    }
  };
  try {
...
function pageOn(c) {
  if (!c.row.classList.contains('lumen-descr-row--sub')) return 'main';
  return c.reviews.classList.contains('lumen-descr-page--on') ? 'reviews' : c.fr.classList.contains('lumen-descr-page--on') ? 'fr' : '?';
}

test('второй экран: ↓ с нижнего края основной страницы открывает страницу отзывов', () => {
  const c = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, c.tagB, [], (nav, focusCalls) => {
    const { ctrl, calls } = c.item.controller();
    ctrl.down();
    assert.deepEqual(calls, [], 'штатный down (уход на следующий ряд) не зовётся');
    assert.equal(pageOn(c), 'reviews');
    assert.equal(nav.focus, c.hide, 'фокус — на единственный узел страницы');
    assert.equal(focusCalls[0][1], c.row, 'в коллекции этого ряда');
    assert.equal(c.item.last, c.hide, 'модуль ряда помнит узел отзывов — возврат снизу придёт на него');
  });
  assert.deepEqual(warnLog, []);
});

test('второй экран: пока Navigator может идти сам — страницы не трогаем', () => {
  const c = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, c.text, ['down'], (nav, focusCalls) => {
    const { ctrl, calls } = c.item.controller();
    ctrl.down();
    assert.deepEqual(calls, ['lampa-down']);
    assert.deepEqual(focusCalls, []);
    assert.equal(pageOn(c), 'main');
  });
  assert.deepEqual(warnLog, []);
});

test('второй экран: страницы по порядку и обратно, с краёв — штатный уход из ряда', () => {
  const c = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, c.tagB, [], (nav) => {
    const { ctrl, calls } = c.item.controller();
    ctrl.down();
    assert.equal(pageOn(c), 'reviews');
    ctrl.down();
    assert.equal(pageOn(c), 'fr');
    assert.equal(nav.focus, c.frMode, '↓ — верхний узел страницы (как выбрал бы Navigator)');
    ctrl.down();
    assert.deepEqual(calls, ['lampa-down'], 'с последней страницы — на следующий ряд Lampa');

    ctrl.up();
    assert.equal(pageOn(c), 'reviews');
    ctrl.up();
    assert.equal(pageOn(c), 'main');
    assert.ok(nav.focus === c.tagA || nav.focus === c.tagB, '↑ — нижний узел основной страницы, а не описание');
    ctrl.up();
    assert.deepEqual(calls, ['lampa-down', 'lampa-up'], 'с основной страницы — вверх на шапку карточки');
  });
  assert.deepEqual(warnLog, []);
});

test('второй экран: ↑ на основную страницу — ближайший по горизонтали из нижних узлов', () => {
  const c = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  /* Отзывов нет вовсе: с «Смотреть по порядку» ↑ ведёт сразу на основную. */
  c.holder.children.splice(c.holder.children.indexOf(c.reviews), 1);
  withPagesNav(c, c.frCard2, [], (nav) => {
    c.holder._listeners.filter((l) => l.type === 'hover:focus').forEach((l) => l.fn({ type: 'hover:focus', target: c.frCard2 }));
    assert.equal(pageOn(c), 'fr', 'страница следует за фокусом');
    const { ctrl } = c.item.controller();
    ctrl.up();
    assert.equal(pageOn(c), 'main');
    assert.equal(nav.focus, c.tagB, 'карточка 147…242 ближе к счётчику 140…260, чем к 42…132');
  });
  assert.deepEqual(warnLog, []);
});

test('второй экран: блок без узлов фокуса (скелетон отзывов) — не страница', () => {
  const c = pagesRow({ skeleton: true });
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, c.tagB, [], () => {
    const { ctrl } = c.item.controller();
    ctrl.down();
    assert.equal(pageOn(c), 'fr', 'на скелетон фокус не поставить — сразу следующая страница');
  });
  assert.deepEqual(warnLog, []);
});

test('второй экран: фокус не встал — страница возвращается, нажатие уходит Lampa', () => {
  const c = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, c.tagB, [], () => {
...
test('второй экран: фокус с чужого ряда страницы не трогает', () => {
  const c = pagesRow();
  const other = pagesRow();
  c.item = descrItem();
  LC.header.bindDescr(c.item, c.row, null);
  withPagesNav(c, other.tagB, [], (nav, focusCalls) => {
    const { ctrl, calls } = c.item.controller();
    ctrl.down();
    assert.deepEqual(calls, ['lampa-down']);
    assert.deepEqual(focusCalls, []);
...
test('второй экран: колесо листает страницы только у своего ряда', () => {
  const c = pagesRow();
  c.item = descrItem();
  const wheel = [];
  const link = { scroll: { onWheel: (step) => wheel.push(step) } };
  LC.header.bindDescr(c.item, c.row, link);
  LC.header.bindDescr(c.item, c.row, link);
  withPagesNav(c, c.text, [], (nav) => {
    link.scroll.onWheel(1);
    assert.equal(pageOn(c), 'reviews');
    assert.deepEqual(wheel, [], 'шаг внутри ряда — ряды Lampa не листаются');
    link.scroll.onWheel(1);
    link.scroll.onWheel(1);
    assert.equal(pageOn(c), 'fr');
    assert.deepEqual(wheel, [1], 'с последней страницы колесо идёт к следующему ряду');
    link.scroll.onWheel(-1);
    assert.equal(pageOn(c), 'reviews');
    assert.equal(nav.focus, c.hide);

    Lampa.Controller.enabled = () => ({ name: 'full_descr', controller: { link: {} } });
    link.scroll.onWheel(-1);
    assert.equal(pageOn(c), 'reviews', 'активен ряд описания другой карточки — не наш');
    assert.deepEqual(wheel, [1, -1]);
  });
  assert.deepEqual(warnLog, []);
});
