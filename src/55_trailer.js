  ...
  /*                                                                       */
  /* Публично — LC.trailer.{pickTrailer, modeFor, mode, player, reveal,    */
  /* schedule, stopActive, bind}. Чистая часть (pickTrailer/modeFor) без    */
  /* DOM и без Lampa — тестируется напрямую; рантайм читает window/document/ */
  /* таймеры только ПО ВЫЗОВУ, поэтому модуль грузится обычным              */
  /* test/_load.mjs.                                                        */
  /*                                                                       */
  /* Task 18: у модуля две независимые половины. schedule() — ФОНОВЫЙ       */
  /* беззвучный ролик (ниже), reveal() — показ ШТАТНОЙ кнопки «Трейлер»     */
  /* Lampa, то есть полноценный просмотр со звуком её же плеером. Первая    */
  /* слушается настройки lumen_trailer, вторая — нет (см. reveal).          */
  /*                                                                       */
  ...
  /*  - «ролик РЕАЛЬНО играет» — ПУБЛИЧНЫЙ признак модуля: класс            */
  /*    lumen-trailer-live на слое фона (и парный lumen-trailer-on на       */
  /*    корне карточки). Оба ставит onStart вместе с паузой слайдшоу и      */
  /*    снимает cleanup(). Снаружи читать его следует ТОЛЬКО через          */
  /*    LC.trailer.isLive(layer) — одна точка на всех потребителей          */
  /*    (ревью фазы 1: LC.applySlideshowPref и оба resume() в              */
  /*    LC.onActivityEvent). Живости контроллера для этого НЕ хватает:      */
  /*    schedule() отдаёт живой контроллер сразу, а ролик стартует лишь     */
  /*    через START_DELAY_MS и может не стартовать вовсе (WAIT_MS).         */
  /*                                                                        */
  ...
  LC.trailer = (function () {
    ...
    /* Публичный признак «ролик реально играет» — ЕДИНСТВЕННАЯ точка для всех
       потребителей снаружи (ревью фазы 1, второй круг; см. шапку модуля).
       Класс ставит onStart вместе с паузой слайдшоу и снимает cleanup(),
       поэтому он точнее живости контроллера: та остаётся true и в окне
       ожидания старта, и все WAIT_MS таймаута. Любой мусор на входе (слоя нет,
       пустой набор, не-jQuery узел) — false: «не играет» здесь безопаснее
       исключения, потому что на этом признаке висит возврат ротации кадров. */
    ...
    /* ------------------------------------------------------------------ */
    /* Task 18: показ ШТАТНОЙ кнопки «Трейлер».                            */
    /*                                                                      */
    /* Полноэкранного плеера у нас нет и не будет: он есть у самой Lampa и  */
    /* уже проверен на телевизорах. В app.min.js модуль Trailers (смещения  */
    /* 1429349 и 1433676) на onCreate карточки либо вешает на .view--trailer */
    /* обработчик hover:enter — Select.show со списком всех роликов, а по    */
    /* выбору Player.play(item) + Player.playlist(...) (звук, перемотка,     */
    /* пульт, YouTube-мост player-video__youtube, смещение 694675), либо     */
    /* УДАЛЯЕТ узел кнопки совсем (нет роликов / adult / выключено в         */
    /* lampa_settings.disable_features.trailers). То есть присутствие узла    */
    /* — уже признак «ролики есть», а поведение кнопки писать не нужно.      */
    /*                                                                       */
    /* Невидима кнопка не сама по себе: она лежит в .hide.buttons--container  */
    /* — служебном пуле, из которого Lampa собирает меню кнопки «Смотреть»   */
    /* (onGroupButtons, смещение 1448107). Из пула мы её НЕ вынимаем: ровно  */
    /* от состава пула зависят и это меню, и признак «показывать ли          */
    /* «Смотреть» вообще» (play.toggleClass('hide', !btns.length)), и хэш    */
    /* приоритетной кнопки. Показ делает CSS (30_css.js) по классу корня —   */
    /* разметка кнопки остаётся байт в байт прежней, хэши целы.             */
    /*                                                                       */
    /* Класс ставится по pickTrailer, а НЕ по mode(): настройка              */
    /* lumen_trailer управляет фоновым роликом, а кнопка — осознанное        */
    /* действие пользователя и обязана работать и при lumen_trailer=off.     */
    function reveal(root, data) {
      try {
        if (!root || !root.length) return false;
        var videos = data && data.videos && data.videos.results;
        /* Оба условия обязательны: узел мог быть удалён Lampa (тогда         */
        /* показывать нечего), а ролик без key не сыграет и у неё —          */
        /* pickTrailer как раз такие и отбрасывает.                          */
    ...
    /* Одна подписка на корень карточки: события Lampa (hover:enter) не
       ...
       Task 18 — связка с полноценным просмотром: этот же перехватчик
       обслуживает и кнопку «Трейлер». Отдельного обработчика ей не нужно:
       фаза перехвата идёт ДО штатного hover:enter Lampa, поэтому к моменту,
       когда открывается её список роликов, фоновый iframe уже уничтожен
       (kill -> cleanup: сняты сторож, метка, кнопка «Стоп», возвращено
       слайдшоу) — беззвучный ролик не переживает переход в плеер. Своего
       OK-перехвата на карточке мы намеренно НЕ вводим: кнопка «Смотреть» и
       ряд серий обязаны и дальше вести к самому фильму. */
    function bind(root) {
      try {
        var el = root && root[0];
