  ...
  LC.cardinfo = (function () {
    ...
    /* Правка 2026-09-23 (разбор композиции, п.2.1): длинное название режется
       ...
       Разделители — двоеточие, за которым идёт пробел, и тире, окружённое
       ...
       Ф3 п.3 (ревью фикс-раундов): двоеточие бывает частью самого имени.
         - «Re:ZERO – Жизнь с нуля…» резалось на «Re» / «ZERO – …»: поэтому
           двоеточие без пробела после разделителем не считается.
         - «Mission: Impossible - Fallout» резалось на «Mission» /
           «Impossible - Fallout»: если перед первым двоеточием ОДНО слово,
           а дальше есть тире, режем по тире — «Mission: Impossible» /
           «Fallout». Два слова перед двоеточием («Star Wars: Episode II -
           Attack of the Clones») — это и есть имя, правило не срабатывает.
         - «Mission: Impossible III» тире нет, и текстом его не отличить от
           «Матрица: Перезагрузка». Поэтому слово и слово (номер — арабский
           или римский — словом не считается) по уровням не разносятся:
           неверный разрез хуже, чем фолбэк длинного названия.

       Выборка живых названий TMDB (2026-09-23, через прокси Lampa): ru-RU и
       en-US, popular и top_rated по три страницы, trending за неделю по две,
       японская анимация по три страницы, корейские и китайские по две — для
       кино и сериалов — и 28 коллекций франшиз; 1322 карточки, из них 501
       длиннее 18 символов. Прежнее правило резало 253 названия, новое —
       242. Отличий 17: 6 разрезов исправлены (Re:ZERO по-русски и пять
       «Mission: Impossible» с тире), 4 неверных сняты (Re:ZERO по-английски
       и «Mission: Impossible», II, III), 7 честных ушли в фолбэк («Матрица:
       Перезагрузка», «Терминатор: Генезис», «Pinocchio: Unstrung»…). Среди
       оставшихся 242 неверный на глаз один: «Puella Magi Madoka Magica the
       Movie -Walpurgisnacht: Rising-» — дефисы-скобки японских
       подзаголовков текстом не различить.
       ...
       карточке высоты там, где проблемы нет. Точка и предлог
       разделителями не считаются: «Часть первая» после точки — это
       продолжение названия, а не его второй уровень. */
    var TITLE_SEP = /:\s|\s[—–-]\s/g;
    var NUMBER_WORD = /^(\d+|[IVX]+)$/;

    /* Слова строки без номеров: «Impossible III» — одно, «007» — ни одного. */
