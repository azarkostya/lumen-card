    ...
    try {
      ...
      if (e.type === 'start') {
        ...
        /* Task 19 (fix-раунд итогового ревью фазы 2): чипы настроения
           ...
           Правка 2026-09-17 (второй круг): порядок относительно героя больше
           не обязателен — блок .lumen-moods стал собственным узлом корня и
           живёт при выключенном герое тоже. */
        try {
          if (LC.moods) {
  ...
  /* Правка пользователя 2026-09-17 (п.2): сменили размер кадра над рядами.
     ...
     Task 36 (ревью, находка К1): чипы настроения теперь от героя ЗАВИСЯТ —
     при живом кадре они лежат в его слоте .lumen-hero__moods, без кадра
     монтируются своим узлом в корень активности (src/49_moods.js). Значит
     смена размера меняет под ними место: «Выключен» уносит слот вместе с
     узлом героя, возврат к любому размеру создаёт слот заново и пустым.
     Отсюда два требования, и оба обязательны:
       - чипы монтируются ПОСЛЕ работы с героем, а не до неё: до неё DOM ещё
         в старом состоянии, и LC.moods увидел бы не то место;
       - в обеих ветках, включая «Выключен»: раньше ветка выключения уходила
         в return раньше монтирования.
     Само решение «переехали чипы или нет» принимает LC.moods.mount — его
     гард сверяет фактическое место с записанным, поэтому лишний вызов здесь
     остаётся no-op, как и был. */
