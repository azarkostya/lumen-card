  ...
  LC.cardmenu = (function () {
    ...
    /* Что делать с ответом роликов. 'play' — ещё нужен: запрос последний,
       ...
       плеер не открыт, поверх открыто ровно то же, что при запросе, и ответ
       ...
       меню, — чужой.
       Ревью раунда хвостов, п.1: чужой — оверлей, которого не было при
       запросе, а не любой открытый. Карточки результатов поиска лежат ПОД
       ...
       app.min.js:7027-7033) и в билет не попадает.
       Ревью правок раунда хвостов, п.2 (решение координатора): чужой и
       ответ, до которого закрылся оверлей, открытый при запросе: из поиска
