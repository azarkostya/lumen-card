  ...
  LC.cardmenu = (function () {
    ...
    /* Что делать с ответом роликов. 'play' — ещё нужен: запрос последний,
       ...
       плеер не открыт и после запроса поверх ничего не открылось, и ответ
       не старше TRAILER_WAIT_MS. «Плеер» и «поверх» — тот же набор
       LC.util (playerOpen/overlays), что у автотрейлера героя (ревью волны
       1b, п.4: свой набор здесь разошёлся с ним — поиск, список выбора,
       модальное окно и YouTube Lampa трейлер из меню пропускал), плюс
       левое меню (класс body menu--open, :9789): главную под ним видно, и
       ролик героя его не ждёт, но ответ, пришедший, когда человек уже в
       меню, — чужой.
       Ревью раунда хвостов, п.1: чужой — только оверлей, которого не было
       при запросе (newOverlay). Карточки результатов поиска лежат ПОД
       поиском: search--open стоит и при выборе «Трейлера», и при ответе, и
       проверка «открыт ли хоть какой-то оверлей» глушила такой трейлер
       всегда — ни плеера, ни уведомления (регрессия f3cb272; до неё он
       играл). Список выбора к onSelect уже закрыт (Select.goclose -> hide,
       app.min.js:7027-7033) и в билет не попадает. Оверлей, закрытый после
       запроса (из поиска вернулись на главную), чужим не считается — как и
       до f3cb272: экран тот же, и человек трейлер просил.
