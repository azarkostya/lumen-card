  ...
  LC.color = (function () {
    ...
    /* Чтение пикселей уменьшенной копии. SecurityError бросает getImageData,
       ...
       «настройка выключена» — ровно с этим пользователь и пришёл. Лог
       единичный: до расчёта дело доходит только после трёхсекундной паузы
       фокуса (src/48_hero.js), потоком такие строки не идут. */
    ...
    function fromImage(url, cb, alt) {
      ...
      function done(rgb) {
        if (!live) return;
        release();
        /* Task 35: в кэш идёт только удача. Прежде запоминался и отказ, и
           тогда одна неудачная загрузка (сеть моргнула, прокси ответил 502)
           закрывала постеру дорогу к цвету до конца сеанса — навсегда с
           точки зрения пользователя. Повторная попытка стоит запроса,
           который у браузера почти всегда уже в HTTP-кэше. */
  ...
  LC.accent = (function () {
    ...
    /* Гейт режима анимаций. Ревью фазы 3 (Important 2) требовало здесь
       ровно 'full', и причина была верной: каждое применение пересобирало
       всю таблицу стилей плагина. Task 35 эту причину убрал — смена
       доминанты теперь переписывает один маленький <style> (три правила,
       paint ниже) и ничего не анимирует, так что «лёгким анимациям»
    ...
    /* Task 35: цвет подложки живёт в СВОЁМ <style>. Прежде каждая смена
       ...
       правила, которые от доминанты и зависят (LC.accentCss, src/30_css.js).
      ...
      try {
        ...
        if (!node) {
          ...
          /* Переезд в конец <head> — только по просьбе restyle(), то есть
             ...
             узлы не двигаются: там меняется только текст трёх правил. */
          document.head.appendChild(node);
        }
    ...
    /* Пересчёт и запись узла подкраски по текущей доминанте. Правила строит
       ...
       сборка и быстрая перекраска не разъехались текстом. Доминанты нет или
       плагин выключен (своего CSS у него тогда в head нет вовсе) — узел
       снимается. */
    function paint(last) {
      var rules = '';
