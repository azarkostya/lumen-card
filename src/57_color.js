  ...
  LC.accent = (function () {
    ...
    /* Task 60, жалоба пользователя: «надо сделать это плавным очень, без
       ...
       Расходиться цветам при этом почти негде: тон у них на каждом шаге
       один, а отстать на ОДИН шаг может только подложка рядов — CSS-переход
       .1s стоит лишь на ней, градиенты встают сразу. Величина отставания —
       один шаг пути, то есть около 1/16 разницы двух доминант; на глаз это
       единицы из 255.
       ...
       не по яркой дуге.
       Плата за это — перезапись узла подкраски (около 640 байт при
       настройках по умолчанию: 627-647 на разных цветах, замер на стенде
       2026-09-21) шестнадцать раз за переход.
    ...
    /* Гейт режима анимаций — общий для расчёта (on ниже), подкраски фона
       ...
       мгновенный — там это одна запись, а не шестнадцать. А на ТВ это и есть рабочий режим:
