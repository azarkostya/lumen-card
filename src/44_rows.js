  ...
  LC.rows = (function () {
    ...
    /* Task 39 (решение, чтобы следующий не искал): src готового <img> в
       ...
       Task 74 (настройка «Постеры карточек») этого решения НЕ отменяет, а
    ...
    /* Ряд короче DEDUPE_MIN карточек не показывается вовсе: на экране
       ...
       подборки, короткие по составу: «Звёздные войны» — 11 фильмов, и
       потеря трёх из них в дублях не повод убирать весь ряд с главной.
    ...
    function dedupeAcross(rows, seen, min, fit) {
      ...
      /* Порог длины — отдельным проходом, после окна, и ТОЛЬКО к рядам, из
         ...
         ЭТОГО осталось меньше половины карточек» (src/80_settings.js) — к ним не
      ...
      return function (params, screen) {
        ...
        return function (call) {
          ...
          function finish() {
            ...
            /* Task 74: первым аргументом null — у ряда адвента нет ОДНОЙ
               ...
               картинки по id карточки; режим «оригинал» переспрашивает
        ...
        return function (call) {
          ...
          var handle = LC.sources['fetch'](
            ...
            function (json) {
              ...
              /* Task 74: подмена постеров — ПОСЛЕ фильтра досмотренного и
