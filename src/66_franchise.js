  ...
  LC.franchise = (function () {
    ...
    /* Task 39: ширина постера части в em Lampa — .lumen-fr-card{width:7.90em}
       ...
       width:100%). Размер файла выбирает LC.util.posterSize по этой ширине,
       а не зашитый w300 (он был крупнее нужного на Full HD и мельче нужного
       при DPR 2).
       Task 68 (замер на стенде, 24 клетки): при размерах интерфейса по
       умолчанию это 180 физических пикселей и на 1920 CSS px при DPR 1, и
       на 960 при DPR 2 — оба множителя учитывает LC.util.emPx, а
       произведение «ширина окна × DPR» у обоих целевых экранов одинаковое.
       Размах по всем клеткам — 146-227 px; именно этот ряд и оказался
       единственным местом, где прежний допуск 0.85 давал растяжение
       (216 px из w185, то есть 1.17), см. FIT_POSTER в src/10_util.js. */
    var POSTER_EM = 7.90;
