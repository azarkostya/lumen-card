  ...
  LC.hero = (function () {
    ...
    function onFocus(el) {
      ...
      /* Task 37: фокус не сменился — обновляем только источник перехода и
         ...
         возвращает фокус на место (Controller.collectionFocus зовёт
         Navigator.focus на прежнем элементе, app.min.js:46481 → 46438), и
    ...
    /* Task 37: фокус ловится нативным слушателем в фазе ЗАХВАТА на корне
       ...
       Navigator.focus снимает класс focus со ВСЕЙ коллекции навигации, обходя
       её в цикле, и ставит его на цель (removeClass(['focus']) + toggleClass,
       app.min.js:46441-46442, сам обход — :46422-46429; в ряду главной это
       столько же мутаций, сколько в коллекции селекторов, то есть десятки).
       Сверх того колбэк срабатывал на любой наш собственный класс,
       поставленный внутри того же корня (узел героя лежит в нём), снова
       находил карточку с классом focus и повторял весь onFocus — а значит и
       перезапускал таймеры акцента и трейлера. Слушатель события даёт ровно
       один вызов на один перевод фокуса. */
    ...
    /* Карточка, которая уже в фокусе на момент монтирования (возврат из
       ...
       'hover:focus' при этом может не быть вовсе: класс focus на карточке
       уже стоит с прошлого раза). */
