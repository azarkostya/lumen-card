  ...
  LC.hero = (function () {
    ...
    function render(model, swap) {
      ...
      function write() {
        ...
        /* Раунд «Цвет сразу»: текст нового фильма на экране — в тот же миг
           ...
           же показа (детали, логотип) цвет не трогают. Цвета ещё нет —
           прежний стоит, пока он не посчитается, и новый встаёт одной
           сменой. */
    ...
    /* Ревью H4: «Назад» из карточки. Рантайм на старте главной снимает
       ...
       Сразу: это возврат, а не листание, и цвет постера, на котором
       открывали карточку, уже в кэше LC.color. */
    function accentBack() {
      if (!state || state.parked) return;
