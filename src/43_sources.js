  ...
  LC.sources = (function () {
    ...
    /* Проверено по данным: первые страницы всех 111 discover-источников    */
    /* манифеста (2165 карточек, стенд 2026-09-23) — жанр 10767 стоит у 28, */
    /* и все 28 — ток-шоу, вечерние шоу, церемонии и подкаст («Шоу Грэма    */
    /* Нортона», «Ночное шоу с Джимми Фэллоном», «Церемония вручения премии */
    /* «Тони»», Skavlan…); ни одного игрового сериала или фильма. У фильмов */
    /* этого жанра не бывает вовсе — он есть только в списке жанров TV.     */
    /* Соседние признаки отделяют хуже и не взяты: «News» (10763) стоит и   */
    /* у документального сериала (Vetenskapens värld, 99,10763), «Reality»  */
    /* (10764) — у конкурсов, которые смотрят (Idol, «Танцы со звёздами»),  */
    /* video:true у фильмов в выборке не встретился ни разу, а пустой       */
    /* vote_count бывает и у ещё не вышедших сериалов.                      */
    /*                                                                     */
    /* Фильтр — на стороне TMDB (without_genres), а не выбрасыванием из     */
    /* ответа: так страницы остаются полными, а тот же запрет доезжает и до */
    /* полного списка подборки (discoverUrl → category_full Lampa), где     */
    /* выбрасывать было бы уже нечем. Касается только discover по TV:       */
    /* коллекции и списки TMDB — это ручной отбор, а у фильмов жанра нет.   */
    /* ------------------------------------------------------------------ */

    var TV_WITHOUT = '10767';

    /* filter{} источника с добавленным запретом для TV; входной объект не
       меняется (он из манифеста). Свой without_genres у источника
       сохраняется — список через запятую TMDB читает как «ни одного из». */
