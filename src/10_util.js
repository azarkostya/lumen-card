  ...
  LC.util = (function () {
    ...
    /* Поверх экрана открыто то, что Lampa сама закрывает, возвращаясь к
       ...
       хвостов, п.2): ambience--enable ставит и сам поиск, и «открыт ли хоть
       что-то» по нему глушило бы всё под поиском. Их спрашивает автотрейлер
       героя (homeHidden в src/48_hero.js), а ambience--enable — ещё и
       трейлер меню карточки, своей сверкой с билетом запроса (overlaysNow в
       src/63_cardmenu.js, ревью правок раунда хвостов, п.3). Читаем голый
       DOM: jQuery здесь не нужен.
