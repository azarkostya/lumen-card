  ...
  function palette() {
    ...
    /* Правка пользователя 2026-09-17 (третий круг): «фон определялся от
       ...
       тон подхватывают разом все подложки плагина и затемнение кадра героя, и ни одному
  ...
  function accentRules(P, t) {
    ...
    return {
      ...
      /* Волна 3 (проверка на ТВ 2026-09-24): затемнение кадра героя — три
         ...
         P.bgRgb и потому живут в наборе подкраски: оттенок фона от постера
         доезжает до них тем же узлом, что и до .lumen-main. Числа и их
         разбор — у констант SCRIM_* / FLOOR_* ниже по файлу.
  ...
  /* Волна 3 (проверка на ТВ 2026-09-24): затемнение кадра героя.
     ...
     слое, цвет P.bgRgb из набора подкраски (accentRules выше); вуалей в
     потоке текста и масок на самих картинках больше нет.
     ...
     3. Левое затемнение (scrimLeft, SCRIM_L) — подушка под текстом. По
        ...
        краю текста (не дальше 52 % ширины: блок TEXT_MAX_W кегля текста от
        safe area — 491 CSS px на 960) плотность не ниже .84, и мета с
        описанием (P.muted) на белом кадре дают там не меньше 4.5:1 —
  ...
  /* Ширина содержимого текстового блока, в его же кегле.
     ...
     только до 52 % ширины: 36em кегля блока от safe area — это 3.19 + 36 =
     39.19em, 491.7 CSS px из 960, то есть 51.2 %. Шире блок не имеет права
     стать, пока не пересчитано затемнение (тест «мета и описание героя
     читаются на белом кадре»). Логотип в блок помещается: его рамка по
  ...
  LC.buildCss = function () {
    ...
    /* Порядок слоёв держит порядок узлов в разметке (buildStage): подложка
       ...
       ролик — ниже затемнения. z-index никому не задан намеренно.
    ...
    /* Затемнение кадра (разбор чисел — у констант SCRIM_* и FLOOR_*): все
       ...
       читается. Пары -webkit-/без префикса — как у остальных масок плагина.
