  ...
  /* Task 35: правила ГЛАВНОЙ, которые меняются вместе с постером под
     фокусом, — от подкрашенного фона (подложка рядов, левая вуаль героя и,
     ...
     Task 64: нижней вуали героя в этом наборе больше нет — она перестала
     быть плашкой и стала градиент-маской самих слоёв кадра (правило маски
     ниже по файлу). Красить в маске нечего, а поверхность, с которой
     встречается подкрашенная подложка рядов, теперь одна и та же:
     .lumen-main из правила main — первого в этом же наборе. Оттенкам
     разъехаться негде, потому что цвет ровно один.
  ...
  function accentRules(P, t) {
    return {
      ...
      /* Правка 2026-09-23 (разбор композиции, п.1.4): плотность поднята и
         растянута вправо — до .97 у левой кромки, .94 к 64 % ширины, ноль к
         78 %. Причина — замер по WCAG 2.1 на белом кадре (а «самый светлый
         кадр» и есть белый: у 60 разобранных кадров TMDB 90-й перцентиль
         яркости в полосе меты доходит до 0.995): прежняя пара .85/.45/0
         давала мета-строке 1.18:1 в худшей точке при норме 4.5, потому что
         строка тянется до 64.3 % ширины, а вуаль на этой отметке кончалась.
         Теперь худшая точка меты — 6.5:1.
         Вертикальную границу вуали задаёт её маска (правило в таблице
         ниже): вверх кадра затемнение больше не идёт. */
      ...
      /* Верхняя вуаль под шапкой Lampa — тот же цвет, что у левой, и по той
         же причине в наборе подкраски: обе лежат на кадре и обязаны
         оставаться одного тона с фоном страницы. Геометрия — в таблице. */
  ...
  /* Ряды в СТАРТОВОМ состоянии опущены на эту долю экрана и поднимаются к
     ...
     12.01 % высоты кадра.
     Это задумано и безопасно. Маска кадра (heroMask ниже по файлу, стопы
     после правки 255f911: .10 на 6 % и .42 на 16 % высоты кадра) оставляет
     на этой высоте .10 + 6.01/10 × .32 = .29 картинки — заметно больше
     прежних .13, — но заголовок ряда лежит в левой части экрана, под
     подушкой-вуалью текста героя (.97 у левой кромки, .94 к 64 % ширины), а
     та в покое тянется до самого низа кадра. Контраст заголовка на белом
     кадре считает тест «мета и топбар героя читаются на белом кадре».
  ...
  LC.buildCss = function () {
    ...
    /* Task 64: переход opacity в полном режиме есть и без тяжёлых эффектов —
       ...
       случая: самое первое появление кадра (0 → 1, одна картинка) и
       приглушение до .25 под начавшимся роликом (.lumen-hero--trailer ниже).
       Task 70: третьего случая — гашения при уходе фокуса в ряды — больше
       нет, правило .lumen-hero--compact снято решением пользователя. */
    ...
    /* Task 70. Здесь стояли четыре правила Task 64: в сжатом состоянии
       гасли оба слоя кадра, подложка LQIP, играющий ролик и левая вуаль.
       Довод был взят из ресёрча (docs/research/2026-09-21-tv-design-specs.md
       §1, «Герой / Top Shelf»: «при уходе фокуса вниз фон убирается
       полностью»), и он ОТМЕНЁН решением пользователя 2026-09-21 — отзыв
       п.1 с телевизора: «всё, что ниже первой ленты, не загружает постер с
       героем». Загрузка была ни при чём, кадр гасил CSS; пользователь
       выбрал поведение до Task 64 — кадр остаётся видимым и только уезжает
       вверх (translateY у .lumen-hero--compact выше).
       Вуаль вернулась вместе с кадром: её гашение было нужно ровно потому,
       что гас кадр, — под текстом снова лежит картинка, ради читаемости на
       которой вуаль и заведена.
       Ролик тоже больше не гасится: он лежит на месте кадра, и гасить его
       под видимым кадром значило бы показывать статичную картинку вместо
       начавшегося ролика.
       Что осталось от Task 64 в этом месте — только слой атмосферы ниже. */
    /* Слой атмосферы в сжатом состоянии гаснет и встаёт на паузу, и это
       единственное исключение из решения выше: частицы стоят кадров. Замер
       Task 64 на шаге фокуса по рядам — 120 пропущенных кадров из 120 за
       2 секунды; вернуть их поверх сжатого кадра не за что.
    ...
    /* Переход только у слоя атмосферы: opacity вуали с Task 70 не меняет
    ...
    /* Правка 2026-09-23 (разбор композиции, п.1.4, вторая половина): верхняя
       ...
       1.00:1 — они исчезают совсем (замер по 60 кадрам TMDB: 90-й
       перцентиль яркости в полосе часов доходит до 1.00, то есть «самый
       светлый кадр» и есть белый).

       Высота бокса — LAMPA_HEAD × 2.25 = 9em, из них полная плотность —
       первые 44 % (стопы AR.veilT), то есть 3.96em, и затухание — остальные
       5.04em. Низ глифов часов стоит на 2.7em от верха экрана (замер на
       стенде 960×540@2: 30.8 px при кегле 11.4055), то есть внутри полосы
       полной плотности. Плотность .5 даёт на белом кадре 3.73:1 при пороге
       3:1 для крупного текста и графики (часы — 22.8 CSS px полужирным, это
       large text по WCAG 2.1).
       Ни blur, ни плашки: один линейный градиент, цвет — из подкраски
       (AR.veilT), как у левой вуали. */
    css.push('.lumen-hero .lumen-hero__veil--t{bottom:auto;height:' + round2(LAMPA_HEAD * 2.25) + 'em}');
    css.push(AR.veilT);
    /* Ревью фикс-раунда (п.1): в СЖАТОМ состоянии весь кадр уезжает вверх на
       heroShift (правило .lumen-hero--compact выше), и верхняя вуаль уезжала
       вместе с ним — под шапкой Lampa оставался голый кадр. Замер на стенде
       960×540@2, крупный кадр, «лёгкий»: вуаль −90…12.6 px, часы 10.3…30.8
       px, то есть полоса полной плотности целиком за верхней кромкой, и
       контраст часов на белом кадре падал с 3.73:1 до 1.00:1. Шапка Lampa
       своего фона не имеет (vendor/lampa/css/app.css:1222-1228 — у .head
       только позиция, ширина и z-index), и подложить под неё больше нечего.
       Лечение — встречный сдвиг: вуаль едет вниз ровно на heroShift и в
       сжатом состоянии стоит у верхней кромки экрана, как в покое. Бокс
       остаётся внутри кадра (9em против высоты кадра в доли экрана), его
       overflow:hidden её не режет. transform, а не top: переход только
       композитный, в полном режиме — той же кривой, что у самого кадра, иначе
       на время перехода вуаль отставала бы от шапки. */
    ...
    /* Task 64, п.3 плана: нижняя вуаль — линейный градиент-МАСКА по самому
       ...
       Правка 2026-09-23 (разбор композиции, п.1.3): затухание сжато с 62 %
       высоты кадра до 40 %. Прежние стопы забирали у кадра нижнюю треть
       целиком и по ВСЕЙ ширине — кадр, которому отдано две трети экрана,
       был различим только до половины. Теперь нижняя полоса гасится ровно
       там, где кадр обязан раствориться в фоне рядов, а читаемость текста
       держит не она, а левая вуаль с её собственной маской (правило выше):
       затемнение перестало быть общим на всю ширину и стало подушкой под
       текстом. Замер модели (scratchpad, интеграл видимости кадра по
       площади): кадр различим до 20 % высоты снизу вместо 30 % при x = 80 %
       ширины, то есть на 36 CSS px ниже на стенде 960×540.
       Второй стоп при этом остался на той же плотности .10, а его позиция
       поднялась с 10 % до 6 %: заголовок первого ряда в старте стоит на
       12 % высоты кадра от его низа (замер там же), и на этой высоте от
       картинки теперь остаётся .30 вместо .13 — но лежит заголовок в левой
       трети, под полной плотностью вуали (.97), и его контраст вырос, а не
       упал (16.8:1 против 15.8:1, расчёт — в тесте). */
    ...
    /* Контрольное ревью 84c7b27..de0e2c8, п.3: скелетоны из проявления
       ...
       (src/48_hero.js); цепочка :not() — тоже CSS3. */
    ...
    /* Верхний градиент — замена верхнему стопу маски. Лежит :after'ом внутри
       ...
       верхний em градиента ложится на самый низ кадра, а там кадр уже почти
       растворён в том же P.bg маской своих слоёв (heroMask ниже по файлу,
       Task 64: от 0 у кромки до .10 на 6 % высоты кадра). 1em — это 3–5 %
       высоты кадра (крупный кадр на 960×540 — 11.4 из 360 px, компактный
       на «крупнее» — 12.0 из 243), то есть от картинки там остаётся не
       больше .09, и заливка тем же цветом незаметна. Отдельной нижней
       вуали-плашки в accentRules, на которую здесь ссылалась прежняя
       редакция, с Task 64 нет.
