  ...
  /* Вертикальный зазор между рядами главной. Разбор числа — у правила
     .lumen-main .items-line ниже; здесь константа потому, что из неё же
     считается интервал отношений сторон, в котором зазор подменяется
     расчётным (правило кромки, там же). */
  var ROW_GAP = 1.4;
  ...
  LC.buildCss = function () {
    ...
    /* Правка 2026-09-23 (разбор композиции, п.6 — «кромка экрана имеет право
       резать изображение и не имеет права резать текст»).

       Те самые «3 px строки», о которых говорит комментарий выше, и есть
       дефект: от следующего ряда видна верхушка глифов его заголовка, а
       срезанная строка читается не как «ниже есть ещё», а как ошибка
       рендеринга. Прогон по 72 клеткам раскладки (тот же перебор, что в
       тесте Task 51) показал, что клеток с таким срезом 17: в 49 заголовок
       следующего ряда помещается целиком, в 6 он и так целиком за кромкой,
       а в оставшихся 17 кромка проходит сквозь него.

       Статическим числом это не чинится: срез возникает, когда низ ряда
       попадает в полосу высотой в одну строку заголовка прямо над кромкой,
       и в разных клетках низ ряда стоит от 405 до 543 CSS px. Зазор,
       который убрал бы срез везде, был бы 135 px — это уже не зазор, а
       дыра. Зато сама полоса выражается интервалом отношений сторон, ровно
       как остальные пороги этой таблицы, и внутри него зазор считается
       точно: padding = (100 − ROWS_TOP_VH)vh − (ROWS_AIR + блок ряда)em, то
       есть ряд кончается РОВНО на кромке, а заголовок следующего начинается
       за ней. Вывод интервала: низ ряда = A·H + B·W, где A = ROWS_TOP_VH/100
       и B = (ROWS_AIR + блок + ROW_GAP)/screenEm(); срез идёт, пока
       A·H + B·W < H < A·H + (B + C)·W, где C = высота шапки/screenEm(), то
       есть пока (1−A)/(B+C) < W/H < (1−A)/B.

       Блок ряда здесь — ПОТОЧНЫЙ, без сдвига подписи под фокусом
       (rowBlockEm с flow): сдвиг это transform, и следующий ряд встаёт от
       линии без него. Замер на стенде 960×540@2 сходится с расчётом: низ
       подписи в потоке 520.5 CSS px, прежний зазор давал низ ряда 536.5 и
       заголовок следующего ряда на 539.8…553.8, то есть 0.2 px строки над
       кромкой; расчётный зазор 19.5 px ставит низ ряда ровно на 540.0.

       Интервалов ДВА, и это не симметрия ради симметрии: за порогом узкой
       колонки карточка уже, подписи мельче, ряд короче — и его низ снова
       поднимается в ту же опасную полосу под кромкой. Первая редакция этой
       правки обрезала интервал порогом узкой колонки, считая, что за ним
       резать нечего; модель показала обратное — 10 клеток из 72 остались со
       срезом, и все они были за порогом. Поэтому «широкий» интервал
       обрезается порогом сверху, а «узкий» начинается от него же и считает
       свой блок по узкой раскладке.

       Цена правки названа прямо: «подглядывание» следующего ряда исчезает
       совсем. Комментарий к ROW_GAP выше считал эти 3 px полезными — после
       разбора считаем наоборот, потому что подглядывать текстом нельзя, а
       постеров следующего ряда в эти 3 px всё равно не попадало. */
    ...
    /* Интервал отношений сторон, в котором заголовок следующего ряда попадает
       на кромку, и расчётный зазор для него. tailEm — от верха области рядов
       до низа подписи В ПОТОКЕ. */
