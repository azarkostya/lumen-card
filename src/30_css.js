  ...
  /* Task 60 (ревью): подсветка карточки под фокусом уехала в СВОЙ узел
     <style id="lumen-accent-focus">. Она зависит не от доминанты, а от
     токенов акцента — значит смена доминанты, не сменившая акцента, узел
     не переписывает (тест в test/color.test.mjs). Пока правило лежало
     вместе с фоном, каждая перезапись фона (тогда — шестнадцать на путь
     цвета; с раунда «Цвет сразу» путь — одна запись) заставляла движок
     заново оценивать '.lumen-main .card.focus .card__view' — самый
  ...
  /* live — для узла подкраски (LC.accentCss): ревью 4be39a5, (b) — узел
     переписывается на каждой смене цвета фильма, и все полосы разом
  ...
  LC.buildCss = function () {
    ...
    /* Правка 2026-09-26: правый зазор снят у ПОСЛЕДНЕЙ карточки ряда — при
       числе колонок из «Размера плиток» (стенд: с прибитым 6n при пяти
       колонках пятая карточка переносилась, и в ряду стояло четыре). */
    ...
    /* Правка пользователя 2026-09-17 (третий круг): фон под рядами — это фон
       ...
       режиме — он сглаживал ступеньку между шагами пути цвета. Но фон корня
       ...
       узла — снят. С раунда «Цвет сразу» (2026-09-26) шагов пути нет вовсе:
       LC.accent (src/57_color.js) ставит цвет фильма одной записью, и корень
       меняется вместе с градиентами подкраски. */
    ...
    /* Task 28: слой автотрейлера — поверх кадра, под затемнением. Волна 3:
       ...
       правилом узла, который переписывается на каждой смене цвета фильма;
    ...
    /* Правка 2026-09-26 (пользователь: «будет меньше анимаций»): без сжатия
       ...
       vendor/lampa/css/app.css:2762-2769); герой при этом не двигается.
       Финальная проверка 2026-09-26, B1: снимать этот переход в «лёгком» и
       выключенном режиме НЕЛЬЗЯ. Lampa заканчивает прокрутку только по
       webkitTransitionEnd ленты (Scroll.startScroll, app.min.js:32022-32032):
       из него зовутся scrollEnded → Layer.visible и onAnimateEnd →
       pushLoaded — дописывание следующего ряда. Без перехода события нет, и
       главная застревала на 4 рядах из 10 (стенд: 8 × «вниз» — 4 ряда, ни
       одного Layer.visible). Укорачивать переход тоже нельзя: обработчик
       Lampa отбрасывает событие, пришедшее раньше 300 мс после прошлого
       (`Date.now() - time_call_end < 300`), и при удержании стрелки короткий
       переход успевал бы кончаться на каждом шаге — последнее событие серии
       терялось бы. Штатные .3s отменяются новым шагом и дают одно событие в
       конце серии, поэтому лента рядов остаётся со штатным переходом Lampa
       во всех режимах. */
