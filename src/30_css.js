  ...
  /* Ряды в СТАРТОВОМ состоянии опущены на эту долю экрана и поднимаются к
     ...
     «срезанной наполовину» и встала ровно на кромку; за кромкой она остаётся
     намеренно — там фокус на герое, и ряд подглядывает снизу постерами
     («allow offscreen content to peek in from the edge», docs/research/
     2026-09-21-tv-design-specs.md §1). Цена — textBottomVh: низ текста героя
     в старте поднимается на те же 2.5vh, и оба порога низкого окна
     (textRatio ниже) съезжают пропорционально. */
  ...
  LC.buildCss = function () {
    ...
    /* --- Фаза 3 (долг фазы 2): карточка ряда главной (design-spec-main §0.4) ---
       ...
       держит тест, а не глаз: оно обязано пережить смену ширины карточки.
    ...
    /* Увеличение постера в фокусе. Число живёт в переменной, потому что от
       ...
       16:9 и ×1.14 для постера 2:3, у AOSP (androidx.tv.material3)
