  ...
  LC.buildCss = function () {
    ...
    /* Шапка карточки прижата к НИЗУ кадра (align-items:flex-end при
       ...
       описания ниже — описание и так уже поделило высоту с рядом серий
       (кламп строк у сериала, правило кромки 5601edb). Прижатие к верху
    ...
    /* Правка 2026-09-23 (разбор композиции, пп.2.5 и 6 — «кромка экрана имеет
       ...
       Девять строк — не «сколько влезет», а единственное число, при котором
       кромка не режет НИЧЕГО. От старта 339.2 девять строк вместе с
       паддингами кончаются на 518.8 — запас до кромки 21 px. Восемь строк
       описание бы уместили с запасом 57, но подтянули бы под кромку
       счётчики разделов Lampa («Жанр 3 · Производство 1 · Теги 13»,
       .tag-count под описанием): замер показал их на 527…553.6, то есть
       срез просто переехал бы с описания на них. При девяти строках они
       уходят на 545 — целиком за кромку. Десять строк вернули бы срез уже
       самому описанию (536.8 при кромке 540 — три пикселя запаса, а старт
       описания двигают вещи, которых на этой карточке не было, например
       строка «Продолжить» с таймкодом).
    ...
    /* Правка 2026-09-23 (правило кромки), сериал. У сериала между карточкой и
       описанием стоит ряд серий, и описание начинается ниже: замер на стенде
