  ...
  /* Ряды в СТАРТОВОМ состоянии опущены на эту долю экрана и поднимаются к
     ...
     полоса. 8vh — это 86 px при 1080.

     В старте нижняя часть кадра оказывается ПОД первым рядом: у крупного
     размера кадр кончается на 66.67vh, а заголовок ряда стоит на 58vh плюс
     воздух, то есть примерно в 8 % высоты кадра от его низа. Это задумано и
     безопасно: на такой высоте нижняя вуаль держит около .93 (стопы в
     accentRules выше), и картинки под заголовком ряда практически не видно. */
  var ROWS_SHIFT_VH = 8;
  ...
  LC.buildCss = function () {
    ...
    /* --- Task 25: метки на постерах рядов (главная и сетка подборки) ---
       ...
       левому верхнему. Узкая карточка ряда (11.4em) длинного текста не
