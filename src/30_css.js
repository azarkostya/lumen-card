  ...
  LC.buildCss = function () {
    ...
    /* Шапка ряда меряется по ЗАГОЛОВКУ, а не по кнопке «Ещё», хотя блок ряда
       для порогов считается наоборот — по тому, что выше. Причина в том, что
       зазор один на все ряды страницы, а кнопка есть не у каждого: ряд БЕЗ
       кнопки короче на разницу высот и кончается выше. Считай зазор по ряду
       с кнопкой — и ряд без неё снова упёрся бы заголовком следующего в
       кромку (проверено моделью: 533.5 при кромке 540, ровно эта разница в
       .57em). Считаем по короткому: ряд без кнопки кончается ровно на
       кромке, ряд с кнопкой — на полстроки ниже неё, и заголовок следующего
       за кромкой в обоих случаях. */
    ...
    /* Интервал отношений сторон, в котором заголовок следующего ряда попадает
       ...
       до низа подписи В ПОТОКЕ, без кнопки «Ещё» (почему — абзацем ниже). */
