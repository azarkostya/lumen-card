  ...
  /* Ряды в СТАРТОВОМ состоянии опущены на эту долю экрана и поднимаются к
     ...
     ниже по файлу; между ними интерполяция даёт .08 + 2.01/14 × .32 = .126.
     До Task 64 те же числа лежали в нижней вуали-плашке как .92 и .6
     непрозрачности фона), и картинки под заголовком ряда практически не
     видно.
  ...
  /* Опорное отношение сторон потолка масштаба (×100, как HERO_MIN_RATIO и
     ...
     подписи. Шире 16:9 наш потолок ничего не обещает, и обещать не может:
     дальше в дело идёт порог узкой колонки, а за HERO_MIN_RATIO кадра нет
     вовсе. */
  var TV_RATIO = 178;
  ...
  /* ПОТОЛОК масштаба карточки ряда — из того же бюджета высоты, по которому
     ...
     запаса. На рабочей точке потолка (scale .96) постер узкой колонки
     короче широкого на 2.09em (7.75 против 9.14em ширины × POSTER_RATIO),
     подписи возвращают 0.11em обратно ((1.01 − 0.97) × 2.75 — множители
     подписей из rowBlockEm), итог 1.98em внутри карточки, то есть 2.25em
     блока с множителем «крупнее» (×1.14). Запаса ширины хватает с лихвой,
     и знак разницы ни на одном значении настройки не переворачивается.
