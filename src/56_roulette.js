  ...
  LC.roulette = (function () {
    ...
    /* Карточек на полке: пять 16:9 по 28vh с зазорами — 150vh, то есть они
       ...
       Дизайн-проход 2026-09-26: ширина карточки — пятая часть строки
       (src/30_css.js), и пять ровно заполняют её до правого края барабана.
    ...
    function RouletteComponent(object) {
      ...
      function stopSpin() {
        ...
        try { screen.removeClass('is-spinning'); } catch (e2) { }
      }
      ...
      function paintShelf(list) {
        ...
        /* Размер кадра — по ширине карточки (16:9 от ATV_TILE_VH), тем же
           ...
           Дизайн-проход 2026-09-26: карточка теперь пятая часть строки
           (src/30_css.js) — на ТВ 333 физических пикселя, и w300 тянется на
           11 %. Порог оставлен по высоте: w780 на пять карточек — втрое
           больше декодирования на каждое переключение чипа ради разницы,
           которой с трёх метров не видно. */
      ...
      function spin() {
        if (spinning) return;
        spinning = true;
        /* Дизайн-проход 2026-09-26: метка вращения на обёртке экрана. В виде
           «как Apple TV» по ней притушена колонка слева (src/30_css.js): там
           название и описание главной карточки выборки, и пока барабан
           перебирал другие фильмы, экран говорил о двух фильмах разом.
           Снимает её stopSpin — остановка барабана, пустой результат, уход. */
        try { screen.addClass('is-spinning'); } catch (eS) { }
