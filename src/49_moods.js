/* -------------------------------------------------------------------- */
  ...
  /*                                                                       */
  /* Архитектура:                                                           */
  /*   Блок .lumen-moods — собственный узел в корне активности главной,    */
  /*   последний её ребёнок, и от героя он не зависит: остаётся на экране  */
  /*   при любом размере кадра, включая «Герой: выключен». Место под него  */
  /*   раскладка узнаёт по классу .lumen-moods-on на том же корне          */
  /*   (src/30_css.js). Чипы имеют класс .selector, поэтому                */
  /*   пространственный Navigator Lampa (window.Navigator) находит их      */
  /*   геометрически, без перехвата чужих контроллеров. Замер живьём: шаг  */
  /*   «вверх» с ПЕРВОГО ряда чипам не достаётся — контроллер items_line   */
  /*   отдаёт фокус шапке Lampa сам; так было и когда блок лежал внутри    */
  /*   героя, переезд этого не менял.                                      */
  /*                                                                       */
  /* Навигация:                                                             */
  ...
  LC.moods = (function () {
    ...
    function mount(root) {
      try {
        ...
        /* Правка пользователя 2026-09-17 (второй круг): блок кладётся в сам
           корень активности, последним ребёнком — не внутрь героя. Внутри
           героя он исчезал вместе с ним при «Герой: выключен» и срезался
           кромкой сжатого кадра (overflow:hidden). Последним — чтобы чипы
           рисовались поверх кадра: порядок отрисовки здесь задаёт разметка,
           как и у самого героя. Класс .lumen-moods-on на корне — признак для
           раскладки (src/30_css.js): без героя под полосу чипов опускаются
           ряды, иначе она легла бы на первый из них. */
