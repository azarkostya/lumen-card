  ...
  function slideshowEnabled() {
    /* Task 40: ротация кадров карточки — тяжёлый эффект: каждый кадр это
       новая полноэкранная картинка, её загрузка, декодирование и кроссфейд.
       Тумблер её гасит целиком, не трогая саму настройку слайдшоу: включив
       тяжёлые эффекты обратно, пользователь получает ровно то, что выбрал.
       Функция читается контроллером на каждом resume() (см. шапку
       src/51_slideshow.js), поэтому смена тумблера применяется на лету. */
